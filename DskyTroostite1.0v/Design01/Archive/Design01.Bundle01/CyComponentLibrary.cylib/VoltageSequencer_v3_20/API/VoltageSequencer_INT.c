...
CY_ISR(`$INSTANCE_NAME`_SeqStateMachineIsr)
{
    ...
    ***************************************************************************/
    ...
    /***************************************************************************
    * Rails that have been forced OFF, but have not shutdown because their 
    * power down pre-requisites have not been met.
    ***************************************************************************/
    static `$SeqBitsType` railPendOff = 0u;
    ...
    ***************************************************************************/
    ...
    /*************************************************************************** 
    * Clear pending OFF requests. The sequencer will set them again if any rail
    * has been forced OFF, but did not shutdown.
    ***************************************************************************/
    railPendOff = 0u;
    /***************************************************************************
        ...
        ***********************************************************************/
            ...
            *******************************************************************/
            ...
            else
            {
                if(enPinState != 0u)
                {
                    ...
                    if((pgStatus & `$INSTANCE_NAME`_pgoodOnPrereqList[converterNum]) ==
            ...
            else
            {
                ...
                else
                {
                    ...
                    else /* Fault condition */
                    {
                        ...
                        /*******************************************************
                        * During system debug and bringup, it may be useful to
                        * disable TON_MAX faults. This will allow designers or
                        * technicians to more easily trace the root cause.
                        *
                        ...
                        *enRegList[(converterNum >> 3u)] &= (uint8)~converterMask;
                        ...
                        *******************************************************/
                        ...
                        if(offMode == 0u)
                        {
                            ...
                            for(groupNum = 0u; groupNum < `$INSTANCE_NAME`_NUMBER_OF_GROUPS; groupNum++)
                            {
                                ...
                                *enRegList[groupNum] &= (uint8)~((uint8)(offBits));
                        ...
                        for(sIdx = 0u; 
                           (sIdx < `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS) && (faultGroup != (`$SeqBitsType`)0u);
                            sIdx++)
                        {
                            if((faultGroup & (`$SeqBitsType`)0x01u) != 0u)
                            {
        ...
        case `$INSTANCE_NAME`_ON:
        {
            ...
            *******************************************************************/
            ...
            {
                /* Determine what caused rail shutdown */ 
                if(`$INSTANCE_NAME`_TEST_BIT(`$INSTANCE_NAME`_faultCond, group, converterMask) == 0u)
                {
                    /* Intented shutdown. Check for power down pre-requisite */
                    if((pgStatus & `$INSTANCE_NAME`_pgoodOffPrereqList[converterNum]) != (`$SeqBitsType`)0u)
                    {
                        `$INSTANCE_NAME`_SET_BIT(railPendOff, group, converterMask);
                        abortLoop = 1u; /* Rail cannot go off */
                    }
                }
            }
            else
            {
                abortLoop = 1u; /* No rail goes off */
            }
        ...
        ptimer++;               /* Set ptimer ptr to associated timer */
...
CY_ISR(`$INSTANCE_NAME`_FaultHandlerIsr)
{
    ...
    ***************************************************************************/
    ...
    for (groupNum = 0u; groupNum < `$INSTANCE_NAME`_NUMBER_OF_GROUPS; groupNum++)
    {
        ...
        fault = (uint8)~pgReg & *enReg;
        if (fault != 0u)
        {
            *enReg &= (uint8)~fault;  /* Disable fault master(s)  */
                     ...
                     sIdx++)
                {
                    if ((faultGroup & 0x01u) != (`$SeqBitsType`)0u)
                    {
                        ...
                        if (offMode == `$INSTANCE_NAME`_IMMEDIATE_OFF)
                        {
                            ...
                            `$INSTANCE_NAME`_forceOffModeReg &= (`$SeqBitsType`)~faultSlave;
                        }
                        else /* Soft off */
                        {
