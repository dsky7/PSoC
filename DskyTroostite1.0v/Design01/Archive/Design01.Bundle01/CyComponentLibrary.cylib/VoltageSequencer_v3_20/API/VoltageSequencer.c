...
static void `$INSTANCE_NAME`_InitSystemConfig(void) `=ReentrantKeil($INSTANCE_NAME . "_InitSystemConfig")`
{
    ...
    #if(`$INSTANCE_NAME`_NUMBER_OF_STS_OUTPUTS > 0u)
        for(converterNum = 0u; converterNum < `$INSTANCE_NAME`_NUMBER_OF_STS_OUTPUTS; converterNum++)
        {
    ...
    for(converterNum = 0u; converterNum < `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS; converterNum++)
    {
        ...
        #endif /* `$INSTANCE_NAME`_ENABLE_PGOOD_ON != 0u */
        `$INSTANCE_NAME`_pgoodOnPrereqList [converterNum]     = `$INSTANCE_NAME`_initPgoodOnPrereqList [converterNum];
        `$INSTANCE_NAME`_pgoodOffPrereqList[converterNum]     = `$INSTANCE_NAME`_initPgoodOffPrereqList[converterNum];
...
void `$INSTANCE_NAME`_Enable(void) `=ReentrantKeil($INSTANCE_NAME . "_Enable")`
{
    ...
    *******************************************************************************/
    uint8 `$INSTANCE_NAME`_GetCtlReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetCtlReseqCnt")`
    {
        ...
        if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
        {
            reseqCnt = `$INSTANCE_NAME`_ctlFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
        }
                                                                ...
                                                                `=ReentrantKeil($INSTANCE_NAME . "_GetPgoodOnPrereq")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        pgoodMask = `$INSTANCE_NAME`_pgoodOnPrereqList[(converterNum - 1u)];
    }
                                                                ...
                                                                `=ReentrantKeil($INSTANCE_NAME . "_GetPgoodOffPrereq")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        pgoodMask = `$INSTANCE_NAME`_pgoodOffPrereqList[(converterNum - 1u)];
    }
...
uint8 `$INSTANCE_NAME`_GetTonMaxReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetTonMaxReseqCnt")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        ReseqCnt = `$INSTANCE_NAME`_tonMaxFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
    }
...
uint8 `$INSTANCE_NAME`_GetPgoodReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetPgoodReseqCnt")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        reseqCnt = `$INSTANCE_NAME`_pgoodFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
    }
...
uint8 `$INSTANCE_NAME`_GetOvReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetOvReseqCnt")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        reseqCnt = `$INSTANCE_NAME`_ovFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
    }
...
uint8 `$INSTANCE_NAME`_GetUvReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetUvReseqCnt")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        reseqCnt = `$INSTANCE_NAME`_uvFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
    }
...
uint8 `$INSTANCE_NAME`_GetOcReseqCnt(uint8 converterNum) `=ReentrantKeil($INSTANCE_NAME . "_GetOcReseqCnt")`
{
    ...
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        reseqCnt = `$INSTANCE_NAME`_ocFaultReseqCfg[(converterNum - 1u)] & (uint8)~`$INSTANCE_NAME`_RESP_CNT_MASK;
    }
...
void `$INSTANCE_NAME`_ForceOff(uint8 converterNum, uint8 powerOffMode) `=ReentrantKeil($INSTANCE_NAME . "_ForceOff")`
{
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        ...
        `$INSTANCE_NAME`_forceOffModeReg |= (`$SeqBitsType`)(((`$SeqBitsType`) powerOffMode) << (converterNum - 1u));
        `$INSTANCE_NAME`_forceOffCmdReg  |= (`$SeqBitsType`)(((`$SeqBitsType`) 1u) << (converterNum - 1u));
