...
namespace I2C_LCD_v1_0
{
    public partial class CyCustomCommandsTab : CyTabWrapper
    {
        ...
        {
            dgvRequiredCmds[colReqCmdSize.Index, row].Value = m_params.RequiredCommands[row].m_cmdSize.ToString();
            dgvRequiredCmds[colReqDataSize.Index, row].Value = m_params.RequiredCommands[row].m_dataSize;
            dgvRequiredCmds[colReqCmdByte1.Index, row].Value = CyParameters.NullableByteToHex(
                                                                          m_params.RequiredCommands[row].m_cmdByte1);
            dgvRequiredCmds[colReqCmdByte2.Index, row].Value = CyParameters.NullableByteToHex(
                                                                          m_params.RequiredCommands[row].m_cmdByte2);
            dgvRequiredCmds[colReqCmdByte3.Index, row].Value = CyParameters.NullableByteToHex(
                                                                          m_params.RequiredCommands[row].m_cmdByte3);
            dgvRequiredCmds[colReqCmdByte4.Index, row].Value = CyParameters.NullableByteToHex(
                                                                          m_params.RequiredCommands[row].m_cmdByte4);
            dgvRequiredCmds[colReqCmdDesc.Index, row].Value = m_params.RequiredCommands[row].m_cmdDescription;
            ...
            CyParameters.SetCellReadOnlyState(dgvRequiredCmds[colReqDataSize.Index, row],
                                                       m_params.RequiredCommands[row].m_isDataSizeReadOnly);
        }
        ...
        {
            dgvCustomCmds[colCustEnable.Index, row].Value = m_params.CustomCommands[row].m_enable;
            dgvCustomCmds[colCustCmdSize.Index, row].Value = m_params.CustomCommands[row].m_cmdSize.ToString();
            dgvCustomCmds[colCustDataSize.Index, row].Value = m_params.CustomCommands[row].m_dataSize;
            dgvCustomCmds[colCustCmdByte1.Index, row].Value = CyParameters.NullableByteToHex(
                                                                             m_params.CustomCommands[row].m_cmdByte1);
            dgvCustomCmds[colCustCmdByte2.Index, row].Value = CyParameters.NullableByteToHex(
                                                                             m_params.CustomCommands[row].m_cmdByte2);
            dgvCustomCmds[colCustCmdByte3.Index, row].Value = CyParameters.NullableByteToHex(
                                                                             m_params.CustomCommands[row].m_cmdByte3);
            dgvCustomCmds[colCustCmdByte4.Index, row].Value = CyParameters.NullableByteToHex(
                                                                             m_params.CustomCommands[row].m_cmdByte4);
            dgvCustomCmds[colCustApiName.Index, row].Value = m_params.CustomCommands[row].m_apiName;
        }
        #endregion
        ...
        {
            m_params.RequiredCommands[row].m_cmdSize = CyParameters.ParseNullableByte(
                                                              dgvRequiredCmds[colReqCmdSize.Index, row].Value);
            m_params.RequiredCommands[row].m_dataSize = CyParameters.ParseNullableByte(
                                                              dgvRequiredCmds[colReqDataSize.Index, row].Value);
            m_params.RequiredCommands[row].m_cmdByte1 = CyParameters.ParseHexByte(
                                                              dgvRequiredCmds[colReqCmdByte1.Index, row].Value);
            m_params.RequiredCommands[row].m_cmdByte2 = CyParameters.ParseHexByte(
                                                              dgvRequiredCmds[colReqCmdByte2.Index, row].Value);
            m_params.RequiredCommands[row].m_cmdByte3 = CyParameters.ParseHexByte(
                                                              dgvRequiredCmds[colReqCmdByte3.Index, row].Value);
            m_params.RequiredCommands[row].m_cmdByte4 = CyParameters.ParseHexByte(
                                                              dgvRequiredCmds[colReqCmdByte4.Index, row].Value);
        }

        public void UpdateCustomTableFromUI(int row)
        {
            m_params.CustomCommands[row].m_enable = CyParameters.CellToBool(
                                                              dgvCustomCmds[colCustEnable.Index, row].Value);
            m_params.CustomCommands[row].m_cmdSize = CyParameters.ParseNullableByte(
                                                              dgvCustomCmds[colCustCmdSize.Index, row].Value);
            m_params.CustomCommands[row].m_dataSize = CyParameters.ParseNullableByte(
                                                              dgvCustomCmds[colCustDataSize.Index, row].Value);
            m_params.CustomCommands[row].m_cmdByte1 = CyParameters.ParseHexByte(
                                                              dgvCustomCmds[colCustCmdByte1.Index, row].Value);
            m_params.CustomCommands[row].m_cmdByte2 = CyParameters.ParseHexByte(
                                                              dgvCustomCmds[colCustCmdByte2.Index, row].Value);
            m_params.CustomCommands[row].m_cmdByte3 = CyParameters.ParseHexByte(
                                                              dgvCustomCmds[colCustCmdByte3.Index, row].Value);
            m_params.CustomCommands[row].m_cmdByte4 = CyParameters.ParseHexByte(
                                                              dgvCustomCmds[colCustCmdByte4.Index, row].Value);
            m_params.CustomCommands[row].m_apiName = CyParameters.CellToString(
        ...
        private void CheckAPINameDuplicates()
        {
            ...
            for (int i = 0; i < m_params.CustomCommands.Count; i++)
            {
                for (int j = 0; j < m_params.CustomCommands.Count; j++)
                {
                    if (i != j)
                    {
                        if ((string.IsNullOrEmpty(m_params.CustomCommands[i].m_apiName) == false) &&
                            (m_params.CustomCommands[i].m_apiName.Trim().ToLower() ==
                             m_params.CustomCommands[j].m_apiName.Trim().ToLower()))
                        {
        ...
        private bool RangeCheck(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                currCellValue = cmdRow.m_dataSize;
                isHex = false;
            }
            ...
            {
                currCellValue = cmdRow.m_cmdByte1;
            }
            else if (e.ColumnIndex == columnCmdByte2Index)
            {
                currCellValue = cmdRow.m_cmdByte2;
            }
            else if (e.ColumnIndex == columnCmdByte3Index)
            {
                currCellValue = cmdRow.m_cmdByte3;
            }
            else if (e.ColumnIndex == columnCmdByte4Index)
            {
                currCellValue = cmdRow.m_cmdByte4;
            }
            else
            {
                return true;
            ...
            if (e.ColumnIndex == columnDataSizeIndex || e.ColumnIndex == columnCmdSizeIndex)
            {
                ...
                max = (cmdRow.m_cmdSize.HasValue) ?
                          (CyParamRanges.DATA_SIZE_MAX + 1 - cmdRow.m_cmdSize.Value) : CyParamRanges.DATA_SIZE_MAX;
            }
            else
            {
