...
*
* Side Effects: 
*  The CSD component should be stopped before calling this API.
*
*******************************************************************************/
...
uint32 `$INSTANCE_NAME`_GetLowCp(`$INSTANCE_NAME`_CP_MEASURE_CONFIG *config)
{
	...
	do
	{
		...
		if((sensorRaw > (uint32)`$INSTANCE_NAME`_CHARGE_VALIDATION_ERROR_TOP) ||(sensorRaw < (uint32)`$INSTANCE_NAME`_CHARGE_VALIDATION_ERROR_BOT))
		{
...
uint16  `$INSTANCE_NAME`_ScanSensorCp(const `$INSTANCE_NAME`_CP_MEASURE_CONFIG *config)
{
	...
	{	
	    ...
	    while(0u != (`$INSTANCE_NAME`_CSD_CNT_REG & `$INSTANCE_NAME`_RESOLUTION_16_BITS))
		{
			/* Wait untill scanning is complete */ 
		}
		/* Read data */
