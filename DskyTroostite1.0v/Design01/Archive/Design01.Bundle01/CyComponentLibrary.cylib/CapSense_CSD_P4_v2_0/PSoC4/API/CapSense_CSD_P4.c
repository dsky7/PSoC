...
/* SmartSense functions */
	...
	extern uint8 scanSpeedTbl[((`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT - 1u) / 8u) + 1u];
...
uint8 `$INSTANCE_NAME`_unscannedSnsDriveMode[`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT];
...
#if(0u == `$INSTANCE_NAME`_M0S8CSD_BLOCK_VER)
	((reg32 *) CYREG_HSIOM_PORT_SEL4)	
#endif /* (0u == `$INSTANCE_NAME`_M0S8CSD_BLOCK_VER) */
};
...
{   
	...
	{
		`$INSTANCE_NAME`_unscannedSnsDriveMode[curSensor] = `$INSTANCE_NAME`_CONNECT_INACTIVE_SNS;
	}
    ...
    /* AutoTunning start */
        ...
        `$INSTANCE_NAME`_lowLevelTuningDone = 1u;
			...
			for(curSensor = 0u; curSensor < `$INSTANCE_NAME`_TOTAL_SCANSLOT_COUNT; curSensor++)
			{
				...
				for(curSensor = 0u; curSensor < `$INSTANCE_NAME`_TOTAL_SCANSLOT_COUNT; curSensor++)
			    {
				    `$INSTANCE_NAME`_compensationIDAC[curSensor] = (`$INSTANCE_NAME`_modulationIDAC[curSensor] + 1u) / 2u;
					`$INSTANCE_NAME`_modulationIDAC[curSensor] = `$INSTANCE_NAME`_modulationIDAC[curSensor] / 2u;
			    }
...
__INLINE void `$INSTANCE_NAME`_DisableSensor(uint32 sensor)
{
    ...
    *`$INSTANCE_NAME`_prtSelTbl[port] &= ~(`$INSTANCE_NAME`_CSD_HSIOM_MASK << pinHSIOMShift);
	    ...
	    if(sensorState == `$INSTANCE_NAME`__SHIELD)
		{
			sensorState = `$INSTANCE_NAME`__GROUND;
		}
	#endif /* (`$INSTANCE_NAME`_IS_SHIELD_ENABLE == 0) */
	...
	if(sensorState != (uint32)`$INSTANCE_NAME`__SHIELD)
	{
		...
		{   
		    /* Connected to Ground */
		    inactiveConnect = `$INSTANCE_NAME`_SNS_GROUND_CONNECT;
		}
...
void `$INSTANCE_NAME`_PreScan(uint32 sensor)
{
	...
	#if ((`$INSTANCE_NAME`_TUNING_METHOD == `$INSTANCE_NAME`__TUNING_AUTO) &&\
		 (0 != `$INSTANCE_NAME`_IS_OVERSAMPLING_EN))
	    oversamplingFactor = `$INSTANCE_NAME`_GetBitValue(scanSpeedTbl, sensor);
