...
* Or it acts as a place holder for a data for a Read transaction.
...
void `$INSTANCE_NAME`_CompleteTransaction(void) `=ReentrantKeil($INSTANCE_NAME . "_CompleteTransaction")`
{
    ...
    *******************************************************************************/
    void `$INSTANCE_NAME`_SetSmbAlert(uint8 assert) `=ReentrantKeil($INSTANCE_NAME . "_SetSmbAlert")`
    {
        ...
        #else
            /* To remove unreferenced variable warning */
            assert = assert;
...
* Return:
*  Status: CYRET_xxx value
*          CYRET_SUCCESS  - in case of successful function execution.
...
uint8 `$INSTANCE_NAME`_EraseUserAll(void) `=ReentrantKeil($INSTANCE_NAME . "_EraseUserAll")`
{
    ...
    /* Implement your metod of erasing Operating Memory in Flash in User 
    * section below.
    */
    /* Place your code between "start" and "end" comment blocks */
