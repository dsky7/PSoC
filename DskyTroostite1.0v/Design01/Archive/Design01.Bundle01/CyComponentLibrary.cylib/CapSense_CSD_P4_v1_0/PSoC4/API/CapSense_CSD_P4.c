...
#include "`$INSTANCE_NAME`.h"
    ...
    void `$INSTANCE_NAME`_EnableScanSlot(uint8 slot);
    void `$INSTANCE_NAME`_DisableScanSlot(uint8 slot);
#else
...
void `$INSTANCE_NAME`_Init(void)
{
    ...
    {  
        `$INSTANCE_NAME`_CSD_Clk1_SetDividerValue(`$INSTANCE_NAME`_Clk1DividerVal[snsNumber]);
        `$INSTANCE_NAME`_CSD_Clk2_SetDividerValue(`$INSTANCE_NAME`_Clk2DividerVal[snsNumber]);
    }
#else
    `$INSTANCE_NAME`_CSD_Clk1_SetDividerValue(`$INSTANCE_NAME`_Clk1DividerVal);
    `$INSTANCE_NAME`_CSD_Clk2_SetDividerValue(`$INSTANCE_NAME`_Clk2DividerVal);
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_SetScanSlotSettings(uint8 slot)
{
	...
	`$INSTANCE_NAME`_CSD_IDAC_REG = newRegisterValue;
          ...
          (`$INSTANCE_NAME`_TUNING_METHOD == `$INSTANCE_NAME`_AUTO_TUNING) )
		`$INSTANCE_NAME`_CSD_Clk1_SetDividerValue(`$INSTANCE_NAME`_Clk1DividerVal[slot]);
		`$INSTANCE_NAME`_CSD_Clk2_SetDividerValue(`$INSTANCE_NAME`_Clk2DividerVal[slot]);
    #else
		`$INSTANCE_NAME`_CSD_Clk1_SetDividerValue(`$INSTANCE_NAME`_Clk1DividerVal);
		`$INSTANCE_NAME`_CSD_Clk2_SetDividerValue(`$INSTANCE_NAME`_Clk2DividerVal);
    #endif /* ( (`$INSTANCE_NAME`_MULTIPLE_FREQUENCY_SET) || \
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_EnableSensor(uint8 sensor)
{
	uint32 newRegisterValue;
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_DisableSensor(uint8 sensor)
{
    uint32 port = (uint32) `$INSTANCE_NAME`_portTable[sensor];
    uint8  pinHSIOMShift = `$INSTANCE_NAME`_pinShiftTbl[sensor] * 4u;
...
void `$INSTANCE_NAME`_PreScan(uint8 sensor)
{
	...
	`$INSTANCE_NAME`_PORT4_PC_REG &= ~(`$INSTANCE_NAME`_CSD_CMOD_PC_MASK);
	`$INSTANCE_NAME`_PORT4_PC_REG |= `$INSTANCE_NAME`_CMOD_PC_STRONG_MODE;
	...
	newRegisterValue = `$INSTANCE_NAME`_CSD_CFG_REG;
	newRegisterValue &= ~(`$INSTANCE_NAME`_PRECHARGE_CONFIG_MASK);
	newRegisterValue |= `$INSTANCE_NAME`_CMOD_PRECHARGE_CONFIG;
	`$INSTANCE_NAME`_CSD_CFG_REG = newRegisterValue;
...
#if(`$INSTANCE_NAME`_CMOD_PREGARGE_OPTION == `$INSTANCE_NAME`__CAPPRIOBUF)
	`$INSTANCE_NAME`_PORT4_PC_REG &= ~(`$INSTANCE_NAME`_CSD_CMOD_PC_MASK);
	`$INSTANCE_NAME`_PORT4_PC_REG |= `$INSTANCE_NAME`_CMOD_PC_HIGH_Z_MODE;
#endif /* (`$INSTANCE_NAME`_CMOD_PREGARGE_OPTION == `$INSTANCE_NAME`__CAPPRIOBUF) */
	
	newRegisterValue = `$INSTANCE_NAME`_CSD_CFG_REG;
	newRegisterValue &= ~(`$INSTANCE_NAME`_PRECHARGE_CONFIG_MASK);
	newRegisterValue |= `$INSTANCE_NAME`_CTANK_PRECHARGE_CONFIG;
	`$INSTANCE_NAME`_CSD_CFG_REG = newRegisterValue;
...
void `$INSTANCE_NAME`_Calc_PRS_Resolution(void)
{
    ...
    for(curSensor = 0u; curSensor < `$INSTANCE_NAME`_TOTAL_SENSOR_COUNT; curSensor++)
	{
        ...
        #if ( (`$INSTANCE_NAME`_MULTIPLE_FREQUENCY_SET) || \
              (`$INSTANCE_NAME`_TUNING_METHOD == `$INSTANCE_NAME`_AUTO_TUNING) )
            senseClk = `$INSTANCE_NAME`_Clk1DividerVal[curSensor];
            sampleClk = `$INSTANCE_NAME`_Clk2DividerVal[curSensor];
        #else
            senseClk = `$INSTANCE_NAME`_Clk1DividerVal;
            sampleClk = `$INSTANCE_NAME`_Clk2DividerVal;
        #endif /* ( (`$INSTANCE_NAME`_MULTIPLE_FREQUENCY_SET) || \
                    (`$INSTANCE_NAME`_TUNING_METHOD == `$INSTANCE_NAME`_AUTO_TUNING) ) */
