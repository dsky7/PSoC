...
/* SAE J2602 is disabled */
    ...
    static void `$INSTANCE_NAME`_ProcessMrf(l_u8 frame[]);
...
static volatile l_u8  `$INSTANCE_NAME`_fsmFlags;
    ...
    static l_u16   `$INSTANCE_NAME`_initialClockDivider;
...
*****************************************************/
    ...
    static volatile l_u8   `$INSTANCE_NAME`_tlFlags;
        ...
        static l_u8 `$INSTANCE_NAME`_prevPci;
        ...
        static l_u8 `$INSTANCE_NAME`_rxFrameCounter = 0u;
                ...
                /* Clear response error signal if frame contains RESPONSE ERROR signal */
                    ...
                    if(`$INSTANCE_NAME`_FRAME_PID_MRF == `$INSTANCE_NAME`_framePid)
                    {
                        ...
                        `$INSTANCE_NAME`_ProcessMrf(`$INSTANCE_NAME`_mrfBuffer);
                            ...
                            tmpPci = `$INSTANCE_NAME`_prevPci;

                            if((`$INSTANCE_NAME`_PDU_PCI_TYPE_FF == tmpPci) ||
                                (`$INSTANCE_NAME`_PDU_PCI_TYPE_CF == tmpPci))
                            {
                    ...
                    else if(`$INSTANCE_NAME`_FRAME_PID_SRF == `$INSTANCE_NAME`_framePid)
                    {
                                ...
                                ( 0u != (`$INSTANCE_NAME`_tlFlags & `$INSTANCE_NAME`_TL_TX_REQUESTED)))
                            {
                                ...
                                `$INSTANCE_NAME`_tlFlags &= (l_u8) ~`$INSTANCE_NAME`_TL_N_AS_TIMEOUT_ON;
                            }
...
l_u16 l_ifc_read_status_`$INSTANCE_NAME`(void) `=ReentrantKeil("l_ifc_read_status_" . $INSTANCE_NAME)`
{
    ...
    ***************************************************************************/
    ...
    if(0u != (`$INSTANCE_NAME`_auxStatus & `$INSTANCE_NAME`_STATUS_EDGE_DETECTED))
    {
        ...
        `$INSTANCE_NAME`_auxStatus &= ((l_u8) ~`$INSTANCE_NAME`_STATUS_EDGE_DETECTED);
    ...
    `$INSTANCE_NAME`_status &= ((l_u8) ~`$INSTANCE_NAME`_STATUS_NOT_CHECKED);
...
static void `$INSTANCE_NAME`_EndFrame(l_u8 status) `=ReentrantKeil($INSTANCE_NAME . "_EndFrame")`
{
    ...
    else if(status ==`$INSTANCE_NAME`_HANDLING_SKIP_OVERRUN)
    {
        /* Set Overrun flag */
        `$INSTANCE_NAME`_fsmFlags |= `$INSTANCE_NAME`_FSM_OVERRUN;
    ...
    *******************************************************************************/
    void ld_init(l_ifc_handle iii) `=ReentrantKeil("ld_init")`
    {
        ...
        iii = iii;
            ...
            while(`$INSTANCE_NAME`_txTlStatus == LD_IN_PROGRESS)
            {
                /* Wait until current message will be processed */
            }
                            ...
                            case `$INSTANCE_NAME`_NCS_READ_BY_ID:
                                ...
                                (frame[`$INSTANCE_NAME`_PDU_D1_IDX] <= 63u))     /* User defined identification */
                                {
                                    ...
                                    if(1u == tempStatus)
                                    {
                                        ...
                                        */
 /*                                       for(i = 0u; i < `$INSTANCE_NAME`_FRAME_DATA_SIZE_5; i++)
                                        {
                                            tmpFrameBuff[i] = frame[i + `$INSTANCE_NAME`_FRAME_DATA_SIZE_3];
                                        }*/
                        ...
                        */
                            ...
                            (`$INSTANCE_NAME`_prevPci == `$INSTANCE_NAME`_PDU_PCI_TYPE_UNKNOWN ))
                        {
                            ...
                            */
                            ...
                            if(`$INSTANCE_NAME`_TL_BUF_LEN_MAX >= tmpWord)
                            {
                                /* Get First Frame Length with following two operations */
                                *`$INSTANCE_NAME`_tlLengthPointer = (((l_u16)
                                    frame[`$INSTANCE_NAME`_PDU_PCI_IDX]) &
                                        ((l_u16) ((l_u8) ~`$INSTANCE_NAME`_PDU_PCI_TYPE_MASK)));

                                *`$INSTANCE_NAME`_tlLengthPointer =
                                    ((l_u16) (*`$INSTANCE_NAME`_tlLengthPointer << 8u)) |
                ...
                `$INSTANCE_NAME`_PDU_PCI_TYPE_CF)                                 /* Consecutive Frames detected */
            {
                    ...
                    */
                    if((`$INSTANCE_NAME`_rxTlStatus == LD_IN_PROGRESS) &&
                        ((tmpPci == `$INSTANCE_NAME`_PDU_PCI_TYPE_FF) ||
                            (tmpPci == `$INSTANCE_NAME`_PDU_PCI_TYPE_CF)))
                    {
                        ...
                        */
                        ...
                        if(((`$INSTANCE_NAME`_rxFrameCounter + 1u) == tmpByte) ||
                                (((`$INSTANCE_NAME`_rxFrameCounter + 1u) == 16u) && (tmpByte == 0u)))
                        {
                            for(i = 2u; i < `$INSTANCE_NAME`_FRAME_DATA_SIZE_8; i++)
                            {
                                *`$INSTANCE_NAME`_rxTlDataPointer = frame[i];    /* Get Frame Data */
        ...
        *
        *******************************************************************************/
        l_u8 ld_tx_status(l_ifc_handle iii) `=ReentrantKeil("ld_tx_status")`
        {
            /* To remove unreferenced local variable warning */
            iii = iii;
