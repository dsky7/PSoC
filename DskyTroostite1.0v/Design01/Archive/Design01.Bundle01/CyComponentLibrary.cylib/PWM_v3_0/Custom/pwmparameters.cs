...
namespace PWM_v3_0
{
    ...
    public class CyParameters
    {
        ...
        private ICyDeviceQuery_v1 m_deviceQuery = null;
        private ICyTerminalQuery_v1 m_termQuery = null;
        ...
        {
            this.m_inst = instQuery;
            this.m_deviceQuery = instQuery.DeviceQuery;
        ...
        {
            CyCustErr error;
            return GetValue<T>(paramName, out error);
        }
        public T GetValue<T>(string paramName, out CyCustErr err)
        {
            T value;
            err = m_inst.GetCommittedParam(paramName).TryGetValueAs<T>(out value);
    ...
    public class CyEnum<T>
    {
        ...
        public string Description
        {
            get
            {
                ...
                CyCompDevParam param = m_prm.Inst.GetCommittedParam(m_paramName);
                if (param != null)
                {
                    m_prm.Inst.ResolveEnumParamToDisplay(param, out res);
                }
            ...
            set
            {
                string enumItemName = m_prm.Inst.ResolveEnumDisplayToId(m_paramName, value);
                try
                {
        ...
        public string[] EnumDescriptionsList
        {
            get
            {
                return new List<string>(m_prm.Inst.GetPossibleEnumValues(m_paramName)).ToArray();
            }
        }
