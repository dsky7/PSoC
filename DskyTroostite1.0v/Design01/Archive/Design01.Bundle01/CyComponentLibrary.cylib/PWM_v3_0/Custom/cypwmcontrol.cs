...
namespace PWM_v3_0
{
    public partial class CyConfigureTab : CyTabWrapper
    {
        ...
        {
            SetFrequencyLabel(Convert.ToInt16(m_numPeriod));
        }
        ...
        private void SetFrequencyLabel(int? period)
        {
            ...
            clkdata = m_params.TermQuery.GetClockData(CLOCK_TERMINAL_NAME, 0);
            try
            {
                if (clkdata[0].IsFrequencyKnown)
                {
                    double infreq = clkdata[0].Frequency * Math.Pow(10, clkdata[0].UnitAsExponent);
