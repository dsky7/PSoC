...
namespace MDIO_Interface_v1_0
{
    public partial class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1, ICyShapeCustomize_v1
    {
            ...
            ICyExpressMgr_v1 mgr)
        {
            ...
            editor.AddCustomPage(Resources.AdvancedTabDisplayName, advancedTab, null, advancedTab.TabName);
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            if (parameters.OperationMode == CyOperationModeType.BASIC)
            {
                ...
                {
                    if (customizers[i].OutputName.Equals(string.Concat(query.InstanceName + "_REG.c")) ||
                        customizers[i].OutputName.Equals(string.Concat(query.InstanceName + "_REG.h")))
                    {
            ...
            for (int regSpaceIndex = 0; regSpaceIndex < parameters.m_regSpaceList.m_regSpaces.Count; regSpaceIndex++)
            {
                ...
                sbl[0].AppendLine(string.Format("    #define {0}_{1}_IDX ({2}u)", query.InstanceName, regSpace.Name,
                    regSpaceIndex + 1));
                sbl.Add(new StringBuilder());
                sbl[0].AppendLine(string.Format("    #define {0}_{1}_START ({2}u)", query.InstanceName, regSpace.Name,
                    CyParameters.ConvertDecToHex(regSpace.StartAddress.Value)));
                sbl.Add(new StringBuilder());
                sbl[0].AppendLine(string.Format("    #define {0}_{1}_END ({2}u)", query.InstanceName, regSpace.Name,
            ...
            {
                CyRegSpace regSpace = parameters.m_regSpaceList.m_regSpaces[regSpaceIndex];
                sbl[0].AppendLine(string.Format("#define {0} ({1}u)", GetRegisterPageSizeDefine(query.InstanceName,
            ...
            {
                CyRegSpace regSpace = parameters.m_regSpaceList.m_regSpaces[regSpaceIndex];
                sbl[0].AppendLine(GetRegisterPageDeclaration(query, regSpaceIndex, regSpace, true));
            }
            ...
            for (int regSpaceIndex = 0; regSpaceIndex < parameters.m_regSpaceList.m_regSpaces.Count; regSpaceIndex++)
            {
                ...
                sbl[0].AppendLine(string.Format("extern {0}{1}_REG_CONFIG_STRUCT {2}{1}_{3}_{4}[{5}];",
                    ...
                    regSpaceIndex + 1,
                    GetRegisterPageSizeDefine(query.InstanceName, regSpaceIndex)));
            }
            ...
            for (int regSpaceIndex = 0; regSpaceIndex < parameters.m_regSpaceList.m_regSpaces.Count; regSpaceIndex++)
            {
                ...
                // All init values equal to 0. Initialization is not necessarry, create declaration only
                if (allZeros && regSpace.MemoryType == CyMemoryType.SRAM)
                {
                    ...
                    sbl[1].AppendLine(GetRegisterPageDeclaration(query, regSpaceIndex, regSpace));
                }
        ...
        private static string GetRegisterPageDeclaration(ICyInstQuery_v1 query, int regSpaceIndex, CyRegSpace regSpace)
        {
            return GetRegisterPageDeclaration(query, regSpaceIndex, regSpace, false);
        }
        private static string GetRegisterPageDeclaration(ICyInstQuery_v1 query, int regSpaceIndex, CyRegSpace regSpace,
            bool addExtern)
        {
        ...
        /// <summary>
        /// Gets byte value where 7-4 bits are 0 and 3-0 bits are ClearOnRead, WriteOnly, TriggerOnWrite and 
        ...
        private static byte GetCtlRegValue(CyRegAddress regAddress)
        {
