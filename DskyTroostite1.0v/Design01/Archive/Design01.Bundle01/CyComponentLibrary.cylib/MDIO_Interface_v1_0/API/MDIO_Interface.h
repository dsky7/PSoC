...
/* Structure that contains information of the configuration of each register */
typedef struct
{
    uint16 mask;                /* 16 bit writable mask */
...
#define `$INSTANCE_NAME`_DTE_XS                     (0x05u)
    ...
    /* Bitmap that specifies the data memory type of each register space */
    ...
    /* Bitmap that specifies the data width of each register space */
...
`$RegSpaceInfo`
    ...
    /* The data stored for each page contain the register space memory address
    ...
    */
    ...
    /* Number of DMA channels used in address analyzer block */
    #define `$INSTANCE_NAME`_NUMBER_OF_CHANNELS         (8u)
...
/* Control register */
#define `$INSTANCE_NAME`_CONTROL_REG (* (reg8 *) `$INSTANCE_NAME`_bMDIO_`$CtlModeReplacementString`_CtlReg__CONTROL_REG)
#define `$INSTANCE_NAME`_CONTROL_PTR (  (reg8 *) `$INSTANCE_NAME`_bMDIO_`$CtlModeReplacementString`_CtlReg__CONTROL_REG)
...
/* MDIO Frame processing block FIFO buffers. In basic mode CPU reads/writes MDIO
...
#define `$INSTANCE_NAME`_FP_AUX_CTL1_REG        (* (reg8  *) `$INSTANCE_NAME`_bMDIO_MdioDp_u1__DP_AUX_CTL_REG)
    ...
    #define `$INSTANCE_NAME`_REG_ATTR_REG       \
                    (* (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_CfgReg__CONTROL_REG)
    #define `$INSTANCE_NAME`_REG_ATTR_PTR       \
                    (  (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_CfgReg__CONTROL_REG)
    #define `$INSTANCE_NAME`_MEM_TYPE_REG       \
                    (* (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_MemTypeCfg__CONTROL_REG)
    #define `$INSTANCE_NAME`_MEM_TYPE_PTR       \
                    (  (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_MemTypeCfg__CONTROL_REG)
    #define `$INSTANCE_NAME`_PAGE_WIDTH_REG     \
                    (* (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_DataWidthCfg__CONTROL_REG)
    #define `$INSTANCE_NAME`_PAGE_WIDTH_PTR     \
                    (  (reg8 *) `$INSTANCE_NAME`_bMDIO_Advanced_`$CtlModeReplacementString`_DataWidthCfg__CONTROL_REG)
...
#define `$INSTANCE_NAME`_BCNT_PER       (0x1Fu)
