...
void `$INSTANCE_NAME`_UpdatePhyAddress(void) `=ReentrantKeil($INSTANCE_NAME . "_UpdatePhyAddress")`
{
    ...
    ****************************************************************************/
                                                            ...
                                                            `=ReentrantKeil($INSTANCE_NAME . "_GetData")`
    {
        ...
        for(i = 0u; (i < `$INSTANCE_NAME`_NUMBER_OF_PAGES) && (result != 0u); i++)
        {
            ...
            {           
                ...
                if(numWords <= ((`$INSTANCE_NAME`_pageEndAddr[i] - address) + 1u))
                {
                    ...
                    /* Check whether the DMA transfer is complete */
                    while(0u != (CY_DMA_CH_STRUCT_PTR[`$INSTANCE_NAME`_fwDMA1_Chan].basic_status[0u] & 
                                 CY_DMA_STATUS_CHAIN_ACTIVE))
                    {
                        ; /* Channel is currently being serviced by DMAC */
                    }
                    result = 0u;
                }
            }
                ...
                {   
                    ...
                    #endif /* !`$INSTANCE_NAME`_ALL_PAGES_IN_SRAM */
                    {
                        ...
                        if(numWords <= ((`$INSTANCE_NAME`_pageEndAddr[i] - address) + 1u))
                        {
                            ...
                            #if(CY_PSOC5)
                                while(0u != (CY_DMA_CH_STRUCT_PTR[`$INSTANCE_NAME`_fwDMA2_Chan].basic_status[0u] &
                                             CY_DMA_STATUS_CHAIN_ACTIVE))
                                {
                                    ; /* Channel is currently being serviced by DMAC */
                                }
