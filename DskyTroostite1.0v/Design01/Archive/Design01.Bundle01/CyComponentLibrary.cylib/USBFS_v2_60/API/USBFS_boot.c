...
static uint16 `$INSTANCE_NAME`_universalTime;
                                                            ...
                                                            `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommRead")`
{
    ...
    if(`$INSTANCE_NAME`_started)
    {
        ...
        {
            `$INSTANCE_NAME`_IsConfigurationChanged();  /* Clear configuration changes state status */
    ...
    {
        if(`$INSTANCE_NAME`_IsConfigurationChanged() != 0u) /* Host could send double SET_INTERFACE request or RESET */
        {
            if(`$INSTANCE_NAME`_GetConfiguration() != 0u)   /* Init OUT endpoints when device reconfigured */
            {
                `$INSTANCE_NAME`_CyBtldrCommReset();
            }
        }
