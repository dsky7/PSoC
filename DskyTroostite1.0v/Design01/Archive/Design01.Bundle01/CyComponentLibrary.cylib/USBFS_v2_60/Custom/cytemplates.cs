    ...
    public partial class CyUSBFSParameters
    {
        ...
        private void BuildDescriptorTemplateList(CyDescriptorNode desc, List<CyDescriptorTemplate> lst)
        {
            if (desc.m_value != null)
            {
                ...
                else if (desc.m_value is CyInterfaceAssociationDescriptor)
                    lst.Add(new CyIntAssociationDescriptorTemplate(desc.m_value));
        ...
        public CyUSBDescriptor ImportDescriptor(CyUSBDescriptorType kind)
        {
            ...
            try
            {
                ...
                byte[] byteArray;
                string templateName;
                string serializedstr = LoadTemplateFromFile(kind, out fileExt, out byteArray, out templateName);
        ...
        public bool ImportReport(string fileName)
        {
            ...
            try
            {
                ...
                {
                    serializedstr = LoadTemplateFromFile(CyUSBDescriptorType.HID_REPORT, out fileExt, out byteArray,
                                                         out templateName);
                }
                else
                {
        ...
        private CyDescriptorNode BuildDevTreeFromDesc(List<CyUSBDescriptor> lst, int index, string parentKey)
        {
            ...
            if (node.m_value.bDescriptorType == CyUSBDescriptorType.ALTERNATE)
            {
                ...
                else
                {
                    ...
                    for (int i = configNode.m_nodes.Count - 1; i >= 0; i--)
                    {
                        if (configNode.m_nodes[i].m_value.bDescriptorType == CyUSBDescriptorType.ALTERNATE)
                        {
                            nodeIndex = (byte)i;
                            break;
                        }
                    ...
                    CyDescriptorNode generalInterfaceNode = configNode.m_nodes[nodeIndex];
                    ...
                    node = generalInterfaceNode.m_nodes[generalInterfaceNode.m_nodes.Count - 1];
                }
            }
        ...
        private int BuildReportTreeFromDesc(List<CyHidReportItem> itemList, string rptName)
        {
            ...
            if (itemList != null)
            {
                rptNode = m_hidReportTree.AddNode(m_hidReportTree.m_nodes[0].Key);
                //Report name
                ((CyHIDReportDescriptor)rptNode.m_value).Name = rptName;
                //Items
        ...
        /// </summary>
        /// <returns>ReportTemplate class</returns>
        private static string LoadTemplateFromFile(CyUSBDescriptorType kind, out CyFileExt fileExt,
