...
namespace USBFS_v2_60
{
    public partial class CyHidDescriptorPage : CyEditingWrapperControl
    {
        ...
        {
            get { return CyCustomizer.PAPAM_TAB_NAME_HID; }
        }

        public override IEnumerable<CyCustErr> GetErrors()
        {
            // Save all changes before closing the customizer
            m_parameters.SerializedHIDReportDesc = m_parameters.SerializeDescriptorTree(m_parameters.m_hidReportTree);
        ...
        {
            List<string> templateFiles = m_parameters.GetHIDTemplateFilesList();
        ...
        private void AddItemNode(CyHidReportItem item)
        {
            ...
            if (descNode != null)
            {
                ...
                if ((item.Name == CyHidReportItem.RPTITEM_END_COLLECTION) && (descNode.m_nodes.Count == 0))
                {
