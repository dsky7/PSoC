...
void `$INSTANCE_NAME`_ForceOff(uint8 converterNum, uint8 powerOffMode) `=ReentrantKeil($INSTANCE_NAME . "_ForceOff")`
{
    if((converterNum > 0u) && (converterNum <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        ...
        /* If a rail is forced OFF after a fault condition, clear the fault
        * indication to prevent it from re-sequencing.
        */
        `$INSTANCE_NAME`_faultCond &= (`$SeqBitsType`)~(`$SeqBitsType`)((`$SeqBitsType`)1u << converterNum);
...
void `$INSTANCE_NAME`_ForceAllOff(uint8 powerOffMode) `=ReentrantKeil($INSTANCE_NAME . "_ForceAllOff")`
{
    ...
    /* If rails are forced OFF after a fault condition on any rail(s), clear the
    * fault indication to prevent the faulted rail(s) from re-sequencing.
    */
    `$INSTANCE_NAME`_faultCond = 0u;
