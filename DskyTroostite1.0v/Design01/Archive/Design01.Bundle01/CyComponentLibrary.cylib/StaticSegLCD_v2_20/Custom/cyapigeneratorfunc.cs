...
namespace StaticSegLCD_v2_20
{
    partial class CyAPIGenerator
    {
        ...
        void PutChar14seg_n(TextWriter writer, TextWriter writer_h, int index)
        {
            ...
            writer.WriteLine("    uint8 i;");
            writer.WriteLine("");
            writer.WriteLine("    if(0u == (position / {0}_digitNum_{1}))", m_instanceName, index);
            writer.WriteLine("    {");
        ...
        void WriteString14seg_n(TextWriter writer, TextWriter writer_h, int index)
        {
            ...
            writer.WriteLine("{");
            writer.WriteLine("");
            writer.WriteLine("    while(((uint8) \'\\0\' != *character) && (position != {0}_digitNum_{1}))", m_instanceName, index);
            writer.WriteLine("    {");
        ...
        void WriteBargraph_n(TextWriter writer, TextWriter writer_h, int index, int maxNumber)
        {
            ...
            writer.WriteLine("        }");
            writer.WriteLine("        ");
            writer.WriteLine("        for(i = 1u; i <= {0}_digitNum_{1}; i++) ", m_instanceName, index);
            writer.WriteLine("        {");
