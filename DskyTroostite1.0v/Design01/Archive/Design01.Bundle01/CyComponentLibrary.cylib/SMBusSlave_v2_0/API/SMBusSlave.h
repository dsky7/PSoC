...
#define `$INSTANCE_NAME`_IS_BOOTLOADER_OR_LOADABLE()  \
                /* If communication interface is SM/PM Bus */ \
                ((defined(CYDEV_BOOTLOADER_IO_COMP) && ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_`$INSTANCE_NAME`) || \
                /* Or if communication interface is custom interface */ \
                (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface))) || \
                /* Or project type is bootloadable */ \
                (CYDEV_PROJ_TYPE_LOADABLE == CYDEV_PROJ_TYPE))
...
extern volatile uint8 `$INSTANCE_NAME`_bufferIndex;          /* Index used to navigate through the buffer */
...
/* Defines index of byte count field for block commands */
#define `$INSTANCE_NAME`_BLOCK_BYTE_COUNT_IDX        (0u)
...
/* Constants dedicated for working with FLASH */
...
*              Macros
***************************************/
/* Converts Flash address to Flash row */
