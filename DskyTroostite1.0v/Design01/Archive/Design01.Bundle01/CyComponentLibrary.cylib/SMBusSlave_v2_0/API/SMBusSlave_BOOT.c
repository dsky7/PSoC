...
*
* Side Effects:
*  This function enables component interrupt. If I2C is enabled
...
*  The function updates the I2C read buffer with response and releases it to
*  the host. All reads return 0xFF till the buffer will be released. All bytes
*  are transferred by the I2C ISR.
...
*
* Theory:
*  This function should be called after command was received to unblock the 
*  bus.
...
*  return from this function (to meet manual command handling behavior). When 
         ...
         `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommWrite")`
{
    ...
    *       stretching on the read that follows it. We have missed the
    ...
    */
    ...
    if (`$INSTANCE_NAME`_CMD_UNDEFINED != `$INSTANCE_NAME`_lastReceivedCmd) 
    {
...
*  function checks status with timeout to determine the end of transfer and
