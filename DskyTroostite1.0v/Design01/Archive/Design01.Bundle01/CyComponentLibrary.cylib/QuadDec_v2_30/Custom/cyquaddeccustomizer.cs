...
namespace QuadDec_v2_30
{
    ...
    public partial class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1
    {
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            CyAPICustomizer file_int = null;
            CyAPICustomizer file_pvt = null;
            for (int i = 0; i < customizers.Count; i++)
            {
                if (customizers[i].OriginalName.EndsWith("QuadDec_INT.c"))
                {
                    file_int = customizers[i];
                }
                if (customizers[i].OriginalName.EndsWith("QuadDec_PVT.h"))
                {
                    file_pvt = customizers[i];
                }
            }
            ...
            {
                if (file_int != null)
                    customizers.Remove(file_int);
                if (file_pvt != null)
                    customizers.Remove(file_pvt);                   
            }
            return customizers;
        }
        #endregion
