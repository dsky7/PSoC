...
*    CYRET_SUCCESS                      Function passed successfully
*    `$INSTANCE_NAME`_FAIL              Function failed
*    `$INSTANCE_NAME`_OUT_OF_RANGE      Function parameter is out of range
*
*******************************************************************************/
...
*   Value                              Description
*    `$INSTANCE_NAME`_ROUND_ROBIN       Round robin arbitration
*    `$INSTANCE_NAME`_FIXED_PRIORITY    Fixed priority arbitration.
*
* Return:
*  Indication whether register is written and verified.
*   Define                             Description
...
uint8 `$INSTANCE_NAME`_SetArbiter(uint8 arbiter) `=ReentrantKeil($INSTANCE_NAME . "_SetArbiter")`
{
    ...
    {
        `$INSTANCE_NAME`_CFG_REG.byte[1u] &= (uint8) (~`$INSTANCE_NAME`_ARBITRATION_MASK);    /* Round Robin */
...
*  sm: Sampling Mode.
*   Define                               Description
*   `$INSTANCE_NAME`_ONE_SAMPLE_POINT     one sampling points is used
...
*  reset: Reset Type.
*   Define                             Description
*    `$INSTANCE_NAME`_MANUAL_RESTART    After bus-off, the CAN must be restarted
*                                       by hand. This is the recommended 
*                                       setting.
*    `$INSTANCE_NAME`_AUTO_RESTART      After bus-off, the CAN is restarting 
...
uint8 `$INSTANCE_NAME`_SetRestartType(uint8 reset) `=ReentrantKeil($INSTANCE_NAME . "_SetRestartType")`
{
    ...
    {
        `$INSTANCE_NAME`_CFG_REG.byte[0u] &= (uint8) (~`$INSTANCE_NAME`_RESET_MASK);   /* Manual reset */
...
*  edge: Edge Mode.
*   Define                             Description
*    `$INSTANCE_NAME`_EDGE_R_TO_D       Edge from R to D is used for 
*                                       synchronization
...
*   Define                             Description
*    `$INSTANCE_NAME`_ACTIVE_MODE       Active mode
*    `$INSTANCE_NAME`_LISTEN_ONLY       CAN listen only
*
* Return:
*  Indication whether register is written and verified.
*   Define                             Description
...
uint8 `$INSTANCE_NAME`_SetOpMode(uint8 opMode) `=ReentrantKeil($INSTANCE_NAME . "_SetOpMode")`
{
    ...
    {
        /* CAN in Active mode */
        `$INSTANCE_NAME`_CMD_REG.byte[0u] &= (uint8) (~`$INSTANCE_NAME`_OPMODE_MASK);
...
*    `$INSTANCE_NAME`_BUS_OFF_ENABLE           Busoff State Interrupt Enable
...
*    `$INSTANCE_NAME`_RX_MESSAGE_ENABLE        Msg Recived Interrupt Enable
*
* Return:
*  Indication whether register is written and verified.
*   Define                             Description
