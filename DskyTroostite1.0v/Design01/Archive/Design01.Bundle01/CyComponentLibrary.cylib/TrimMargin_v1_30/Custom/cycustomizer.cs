...
namespace TrimMargin_v1_30
{
    public partial class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1, ICyShapeCustomize_v1, ICyDRCProvider_v1
    {
        ...
        public const string HARDWARE_TAB_NAME = "Hardware";
            ...
            ICyExpressMgr_v1 mgr)
        {
            ...
            {
                // Do nothing. All parameters in Expr View should be read-only
            };

            editor.AddCustomPage(VOLTAGE_TAB_DISPLAY_NAME, voltagesTab, exprDelegate, BASIC_TAB_NAME,
                voltagesTab.TabName);
            editor.AddCustomPage(HARDWARE_TAB_DISPLAY_NAME, hardwareTab, exprDelegate, hardwareTab.TabName);
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            string vNominal = "const uint16 CYCODE " + instanceName + "_VNOMINAL[" + parameters.NumConverters.ToString()
                + "] = { ";
            string vMax = "const uint16 CYCODE " + instanceName + "_VMAX[" + parameters.NumConverters.ToString()
                + "] = { ";
            string vMin = "const uint16 CYCODE " + instanceName + "_VMIN[" + parameters.NumConverters.ToString()
                + "] = { ";
            ...
            string pwmPre = "const " + dutyCycleType + " CYCODE " + instanceName + "_PRE_RUN_DUTYCYCLE[" +
                parameters.NumConverters.ToString() + "] = { ";
            string pwmStartup = "const " + dutyCycleType + " CYCODE " + instanceName + "_STARTUP_DUTYCYCLE[" +
                parameters.NumConverters.ToString() + "] = { ";
            string pwmStartupPre = "const " + dutyCycleType + " CYCODE " + instanceName + "_STARTUP_PRE_RUN_DUTYCYCLE[" +
                parameters.NumConverters.ToString() + "] = { ";
            string pwmPerMV = "const uint16 CYCODE " + instanceName + "_DUTYCYCLES_PER_VOLT[" +
                parameters.NumConverters.ToString() + "] = { ";
            ...
            string preCoef = "const uint16 CYCODE " + instanceName + "_PRE_COEF[" +
                parameters.NumConverters.ToString() + "] = { ";
            ...
            for (int i = 0; i < parameters.NumConverters; i++)
            {
                ...
                UInt16 pwmNomElement = (UInt16)parameters.GetCalculatedPWMNominal(i);
                element = pwmNomElement.ToString();
                AddStringElement(ref pwmNom, element, last);

                // Generate PRE_COEF
                double preCoefValue = (double)(parameters.HardwareTable[i].m_r4 / (parameters.HardwareTable[i].m_r3 +
                ...
                element = Convert.ToUInt16(preCoefValue * 1000).ToString();
                AddStringElement(ref preCoef, element, last);
                ...
                {
                    UInt32 pwmPreElement = Convert.ToUInt32(pwmNomElement * preCoefValue);
                ...
                element = Convert.ToUInt16(parameters.VoltagesTable[i].m_maxVoltage * 1000).ToString();
                AddStringElement(ref vMax, element, last);

                // Generation VMin array
                element = Convert.ToUInt16(parameters.VoltagesTable[i].m_minVoltage * 1000).ToString();
                AddStringElement(ref vMin, element, last);
                ...
                double pwmStartupElement = pwmNomElement - pwmPerMVElement *
                    ((double)(parameters.VoltagesTable[i].m_startupVoltage - parameters.VoltagesTable[i].m_nominalVoltage));
                if (pwmStartupElement < 0) pwmStartupElement = 0;
                if (pwmStartupElement > pwmMax) pwmStartupElement = pwmMax;
                element = Convert.ToUInt16(pwmStartupElement).ToString();
                AddStringElement(ref pwmStartup, element, last);

                UInt32 pwmStartupPreElement = Convert.ToUInt32(pwmStartupElement * preCoefValue);
                if (pwmStartupPreElement > pwmMax) pwmStartupPreElement = pwmMax;
                element = pwmStartupPreElement.ToString();
                AddStringElement(ref pwmStartupPre, element, last);
            ...
            paramDict.Add("VMaxArray", WordWrap(vMax));
            paramDict.Add("VMinArray", WordWrap(vMin));
            ...
            paramDict.Add("PWMStartupArray", WordWrap(pwmStartup));
            paramDict.Add("PWMStartupPreArray", WordWrap(pwmStartupPre));
            ...
            paramDict.Add("preCoefArray", WordWrap(preCoef));
