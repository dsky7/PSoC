...
/* Stores internal component configuration for unconfigured mode */
    ...
    /* Common config vars */
    volatile uint8 `$INSTANCE_NAME`_scbMode = `$INSTANCE_NAME`_SCB_MODE_UNCONFIG;
    volatile uint8 `$INSTANCE_NAME`_scbEnableWake;

    /* I2C config vars */
    volatile uint8 `$INSTANCE_NAME`_mode;
    volatile uint8 `$INSTANCE_NAME`_acceptAddr;

    /* SPI/UART config vars */
    volatile uint8 * `$INSTANCE_NAME`_rxBuffer;
    volatile uint8   `$INSTANCE_NAME`_rxDataBits;
    volatile uint32  `$INSTANCE_NAME`_rxBufferSize;

    volatile uint8 * `$INSTANCE_NAME`_txBuffer;
    volatile uint8   `$INSTANCE_NAME`_txDataBits;
    volatile uint32  `$INSTANCE_NAME`_txBufferSize;
