...
namespace MDIO_Interface_v1_10
{
    ...
    public class CyRegSpace
    {
        ...
        public CyDataWidthType m_dataWidth;
        [XmlElement("MemoryType")]
        public CyMemoryType m_memoryType;
        [XmlElement("ConfigType")]
        public CyMemoryType m_configType;
        [XmlElement("AC")]
        ...
        [XmlIgnore]
        public CyDataWidthType DataWidth
        {
        ...
        [XmlIgnore]
        public CyMemoryType MemoryType
        {
            get { return m_memoryType; }
            set { m_memoryType = value; }
        }
        [XmlIgnore]
        public CyMemoryType ConfigType
        {
        ...
        public CyRegSpace()
        {
            ...
            m_name = DEFAULT_NAME;
            m_dataWidth = CyDataWidthType.BIT_8;
            m_memoryType = CyMemoryType.SRAM;
            m_configType = CyMemoryType.FLASH;
        }
        ...
        public CyRegSpace(int paramIndex, ushort startAddress, ushort endAddress, CyDataWidthType dataWidth)
            : this()
        {
                ...
                for (int addrConfigIndex = 0; addrConfigIndex < regSpace.AddressConfig.Count; addrConfigIndex++)
                {
                    if (regSpace.AddressConfig[addrConfigIndex].InitValue == null ||
                        (regSpace.AddressConfig[addrConfigIndex].Mask == null &&
                         regSpace.MemoryType == CyMemoryType.SRAM) ||
                        ((regSpace.AddressConfig[addrConfigIndex].AuxBits == null) &&
                         (regSpace.MemoryType == CyMemoryType.SRAM) && m_enableAuxBits))
                    {
                        if (sb.Length > 0)
                        {
                            sb.Append(string.Concat(", ", CyParameters.ConvertDecToHex(
                                                   (UInt16)(regSpace.AddressConfig[addrConfigIndex].m_address))));
                        }
                        else
                        {
    ...
    public class CyRegAddress
    {
        ...
        private string GetDisplayValue(string value)
        {
            ...
            try
            {
                string format = "X4";
                ushort uValue = Convert.ToUInt16(value.Replace("0x", ""), 16);
                if (m_parentObject.m_dataWidth == CyDataWidthType.BIT_8)
                {
                    uValue = (byte)uValue;
                    format = "X2";
                }
