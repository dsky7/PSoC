...
namespace MDIO_Interface_v1_10
{
    public partial class CyAdvancedTab : CyEditingWrapperControl
    {
            ...
            : base(param)
        {
            ...
            cbMemoryType.Items.AddRange(CyEnumConverter.GetEnumDescList(typeof(CyMemoryType)));
            cbDataWidth.Items.Clear();
            cbDataWidth.Items.AddRange(CyEnumConverter.GetEnumDescList(typeof(CyDataWidthType)));
            cbConfigType.Items.Clear();
            cbConfigType.Items.AddRange(CyEnumConverter.GetEnumDescList(typeof(CyMemoryType)));
        ...
        private bool RangeCheck(int rowIndex, int colIndex)
        {
            ...
            UInt16 min = 0;
            UInt16 max = (regSpace.DataWidth == CyDataWidthType.BIT_8) ? byte.MaxValue : UInt16.MaxValue;
            string message;
                ...
                if ((currCellValue.HasValue == false) || (currCellValue < min || currCellValue > max))
                {
                    if (colIndex == COL_AUX_BITS)
                    {
                        dgvRegisters[colIndex, rowIndex].ErrorText = string.Format(message,
                           CyParameters.ConvertDecToBin((byte)min), CyParameters.ConvertDecToBin((byte)max));
                    }
                    else if (regSpace.DataWidth == CyDataWidthType.BIT_8)
                    {
                        dgvRegisters[colIndex, rowIndex].ErrorText = string.Format(message,
        ...
        private void UpdateRegSpaceSettingsUI(CyRegSpace regSpace)
        {
            ...
            else
            {
                ...
                cbDataWidth.SelectedItem = CyEnumConverter.GetEnumDesc(regSpace.DataWidth);
                cbConfigType.SelectedItem = CyEnumConverter.GetEnumDesc(regSpace.ConfigType);
                UpdateRegistersTable(regSpace);
                if (cbMemoryType.SelectedItem != null &&
                    cbMemoryType.SelectedItem.ToString() == CyEnumConverter.GetEnumDesc(regSpace.MemoryType))
                {
                    UpdateDataGridAppearance(regSpace);
                }
                else
                {
                    cbMemoryType.SelectedItem = CyEnumConverter.GetEnumDesc(regSpace.MemoryType);
                }
        ...
        private void SetRegSpaceSettings(CyRegSpace regSpace)
        {
            ...
            regSpace.MemoryType = (CyMemoryType)CyEnumConverter.GetEnumValue(cbMemoryType.SelectedItem,
                                                                               typeof(CyMemoryType));
            regSpace.DataWidth = (CyDataWidthType)CyEnumConverter.GetEnumValue(cbDataWidth.SelectedItem,
                                                                               typeof(CyDataWidthType));
            regSpace.ConfigType = (CyMemoryType)CyEnumConverter.GetEnumValue(cbConfigType.SelectedItem,
                                                                               typeof(CyMemoryType));
        ...
        private void cbComboBoxes_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (regSpace != null)
            {
                ...
                {
                    UpdateDataGridAppearance(regSpace);
                    bool readOnly = (regSpace.MemoryType == CyMemoryType.FLASH);
                    if (readOnly)
                    {
        ...
        private void headerCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (m_params.GlobalEditMode)
            {
                ...
                {
                    string newValue = isChecked ? ((regSpace.DataWidth == CyDataWidthType.BIT_8) ?
        ...
        private void UpdateHeaderCheckBoxesState(CyRegSpace regSpace, bool updateHeaderForBooleanFields)
        {
            ...
            UInt16 maxValue = (ushort)((regSpace.DataWidth == CyDataWidthType.BIT_8) ? 0xFF : 0xFFFF);
            for (int addrConfigIndex = 0; addrConfigIndex < regSpace.AddressConfig.Count; addrConfigIndex++)
            {
