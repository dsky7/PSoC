...
void  `$INSTANCE_NAME`_UpdatePhyAddress(void)                   `=ReentrantKeil($INSTANCE_NAME . "_UpdatePhyAddress")`;
    ...
    /* Check if the DMA channel is currently being serviced by DMAC */
    #define `$INSTANCE_NAME`_DMA_ACTIVE(chan)   \
        ((0u != (CY_DMA_CH_STRUCT_PTR[(chan)].action[0u] & CPU_REQ)) || \
         (0u != (CY_DMA_CH_STRUCT_PTR[(chan)].basic_status[0u] & CY_DMA_STATUS_CHAIN_ACTIVE)))
