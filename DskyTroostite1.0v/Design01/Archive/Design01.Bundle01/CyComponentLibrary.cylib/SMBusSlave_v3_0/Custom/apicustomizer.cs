...
namespace SMBusSlave_v3_0
{
    public partial class CyCustomizer : ICyAPICustomize_v1
    {
        ...
        private string GetWriteHandlerCaseBody_PageCmd()
        {
            ...
            sb.AppendFormat("{0}{2}_cmdPage = {2}_buffer[0u];{1}", tabs, NEW_LINE, m_instName);
            sb.AppendLine();
            sb.AppendFormat("{0}/* Copy new page value into Operating Memory Register Store */ {1}", tabs, NEW_LINE);
            sb.AppendFormat("{0}{2}_regs.PAGE = {2}_cmdPage;{1}", tabs, NEW_LINE, m_instName);
            sb.AppendFormat("}}{0}", NEW_LINE);
            return sb.ToString();
        }
        ...
        private string GetWriteHandlerCaseBody_BootloadWriteCmd()
        {
            ...
            string lenType = m_parameters.IsPSoC3 ? "(int16)" : "(uint16)";
            return
