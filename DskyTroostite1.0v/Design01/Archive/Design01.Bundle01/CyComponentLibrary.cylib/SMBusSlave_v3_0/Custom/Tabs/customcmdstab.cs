...
namespace SMBusSlave_v3_0
{
    public partial class CyCustomCmdsTab : CyTabWrapper
    {
        ...
        private void UpdateTypeDependentCells(int rowIndex, object prevValue, object newValue, bool overwriteSize)
        {
            ...
            if (m_params.CustomTable[rowIndex].m_name == CyCustomTable.QUERY)
            {
                readConfigCell.Items.Clear();
                readConfigCell.Items.Add(CyEnumConverter.GetEnumDesc(m_params.CustomTable[rowIndex].m_readConfig));
                readConfigCell.Value = CyEnumConverter.GetEnumDesc(m_params.CustomTable[rowIndex].m_readConfig);
            }
        }
