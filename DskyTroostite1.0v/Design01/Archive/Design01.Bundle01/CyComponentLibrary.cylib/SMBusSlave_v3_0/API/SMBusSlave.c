...
void `$INSTANCE_NAME`_CompleteTransaction(void) `=ReentrantKeil($INSTANCE_NAME . "_CompleteTransaction")`
{
    ...
    *  function contains a merge code section where you may insert code to run
    *  after the Master has responded. For example, you might update a status
    *  register and de-assert the SMBALERT# pin.
    *
    * Parameters:
    *  None
    *
    ...
    *  when it detects a "Receive Byte" protocol request. This function includes
    *  a merge code section where the user may insert their code to override the
    *  default return value of this function - which is 0xFF. This function will
    *  be called in ISR context. Therefore, the user merge code must be fast,
    *  non-blocking, and may only call re-entrant functions.
    *
    * Parameters:
    *  None
    *
