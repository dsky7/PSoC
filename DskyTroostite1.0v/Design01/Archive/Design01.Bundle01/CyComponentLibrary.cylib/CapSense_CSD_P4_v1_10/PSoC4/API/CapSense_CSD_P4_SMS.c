...
extern const uint8 `$INSTANCE_NAME`[];
...
void `$INSTANCE_NAME`_AutoTune(void)
{
    ...
    `$INSTANCE_NAME`_Config.sensetivityTbl = SMS_LIB_SensorSensitivity;
    `$INSTANCE_NAME`_Config.chargeDivTbl = `$INSTANCE_NAME`_Clk1DividerVal;
    `$INSTANCE_NAME`_Config.modDivTbl = `$INSTANCE_NAME`_Clk2DividerVal;
	...
	`$INSTANCE_NAME`_Config.IMOFreq_MHz = CYDEV_BCLK__SYSCLK__MHZ;
    ...
    CalibrateSensors(&`$INSTANCE_NAME`_Config, `$INSTANCE_NAME`_CAL_RAW_COUNT);
	
    TunePrescalers(&`$INSTANCE_NAME`_Config);
	
	#if(`$INSTANCE_NAME`_PRS_OPTIONS != `$INSTANCE_NAME`__PRS_NONE)
		`$INSTANCE_NAME`_PrescalersTuningDone = 1u;
	#endif /* (`$INSTANCE_NAME`_PRS_OPTIONS == `$INSTANCE_NAME`__PRS_NONE) */
	
	`$INSTANCE_NAME`_NormalizeWidgets(`$INSTANCE_NAME`_END_OF_WIDGETS_INDEX, `$INSTANCE_NAME`_Clk1DividerVal);
    ...
    CalibrateSensors(&`$INSTANCE_NAME`_Config, `$INSTANCE_NAME`_CAL_RAW_COUNT_SPLIT);
    
    TuneSensitivity(&`$INSTANCE_NAME`_Config);
...
void `$INSTANCE_NAME`_CalculateThresholds(uint8 sensor)
{
    ...
    CalculateThresholds(sensor, `$INSTANCE_NAME`_SensorRaw, widget, 
