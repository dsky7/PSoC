...
namespace TCPWM_P4_v1_10
{
    ...
    public partial class CyRegisterGrid : UserControl
    {
        ...
        public void UpdateFromParams()
        {
            ...
            if (m_prms.TCPWMConfig == CyTCPWMConfig.PWM_SEL)
            {
                m_period2Cell.Value = m_prms.Period2.ToString();
                m_swapPeriodCell.Value = (m_prms.PeriodSwap == CySwapEnable.SWAP_ENABLE);
            }
            else
            {
            ...
            m_swapCompareCell.Value = (m_prms.CompareSwap == CySwapEnable.SWAP_ENABLE);
        ...
        {
            CySwapEnable value = Convert.ToBoolean(cell.Value)? CySwapEnable.SWAP_ENABLE : CySwapEnable.SWAP_DISABLE;
        ...
        private bool IsCellEnable(DataGridViewCell cell)
        {
            ...
            {
                isEnable = (m_prms.TCPWMConfig == CyTCPWMConfig.PWM_SEL) && 
                           (m_prms.PeriodSwap == CySwapEnable.SWAP_ENABLE);
            }
            ...
            {
                isEnable = (m_prms.TCPWMConfig == CyTCPWMConfig.PWM_SEL) ||
                           (m_prms.CompCapMode == CyCompCapMode.Compare);
            }
            else if (cell == m_compare2Cell)
            {
                isEnable = ((m_prms.TCPWMConfig == CyTCPWMConfig.PWM_SEL) ||
                           (m_prms.CompCapMode == CyCompCapMode.Compare)) &&
                           (m_prms.CompareSwap == CySwapEnable.SWAP_ENABLE);
            }
            else if (cell == m_swapCompareCell)
            {
                isEnable = (m_prms.TCPWMConfig == CyTCPWMConfig.PWM_SEL) ||
        ...
        void dgvRegisters_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if ((e.ColumnIndex == ColSwap.Index) && (e.RowIndex == 0) &&
                (m_prms != null) && (m_prms.TCPWMConfig == CyTCPWMConfig.TIMER))
            {
