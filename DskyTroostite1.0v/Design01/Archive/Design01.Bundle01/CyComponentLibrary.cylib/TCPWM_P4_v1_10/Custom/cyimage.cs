...
namespace TCPWM_P4_v1_10
{
    ...
    class CyImageTC : CyImage
    {
                                            ...
                                            Pen penGrayDash, Brush brushBlack)
        {
            ...
            int compare1Shift = (m_period == 0) ? 0 :
                                (int)((double)m_compare1 / ((double)m_period + 1) * (Y2_COUNTER - Y1_COUNTER));
            int compare2Shift = (m_period == 0) ? 0 :
                                (int)((double)m_compare2 / ((double)m_period + 1) * (Y2_COUNTER - Y1_COUNTER));
            int compare1X = 0;
            int compare2X = 0;
            switch (m_counterMode)
            {
    ...
    class CyImagePWM : CyImage
    {
                                            ...
                                            Pen penGrayDash, Brush brushBlack)
        {
            ...
            // Align dependent
            double add1 = (m_align == CyPWMAlign.CENTER && m_compare1 != 0) ? 0.5 : 0;
            double add2 = (m_align == CyPWMAlign.CENTER && m_compare2 != 0) ? 0.5 : 0;
            if (m_align == CyPWMAlign.RIGHT)
            {
                add1 = 1;
                add2 = 1;
            }
            int compare1Shift = (m_period == 0) ? 0 : (int)((double)(m_compare1 + add1) / ((double)m_period + 1) * Counter1W);
            int compare2Shift = (m_period2 == 0) ? 0 : (int)((double)(m_compare2 + add2) / ((double)m_period2 + 1) * Counter2W);
            ...
            // CC
            if (m_align == CyPWMAlign.CENTER && m_compare1 == m_period)
            {
                DrawArrow(g, penBlack, new Point(X2 + Counter1W, Y_TC + 3 * DEL_H), ARROW_H);
            }
            else
            {
            ...
            if (m_align == CyPWMAlign.CENTER && m_compare2 == m_period)
            {
                DrawArrow(g, penBlack, new Point(PERIOD2_START + Counter2W, Y_TC + 3 * DEL_H), ARROW_H);
            }
            else
            {
            ...
            switch (m_align)
            {
                ...
                case CyPWMAlign.CENTER:
                    if (isAssymetric == false)
                    {
                        ...
                        {
                            //compare1X1 = X2 + Counter1W;
                            compare1X2 = PERIOD2_START;
                            //compare2X1 = PERIOD2_START + Counter2W;
