...
{
    /* Write command pattern for "Set cursor position" to the buffer */
...
static void `$INSTANCE_NAME`_WrDataToBuff(uint8 dByte) CYREENTRANT
{
    /* Put one byte of data to buffer ot locatetion of `$INSTANCE_NAME`_buffIndex */
...
*
* Parameters:
*  cByte:   8-bit value representing the command to be loaded into the command
...
void `$INSTANCE_NAME`_WriteControl(uint8 cByte) `=ReentrantKeil($INSTANCE_NAME . "_WriteControl")`
{
    ...
    {   
        ...
        {
            /* Fill the control pattern based on the found inxed */
            `$INSTANCE_NAME`_WrCntrlPatt(cmdIndx);

            /* Send data to LCD and wait when transaction complete */
            `$INSTANCE_NAME`_SendSequence();

            /* The following commands require 1.5 ms delay to execute */
            switch(cByte)
            {
            ...
            /* The command index was found - break the while() loop */
            break;
        }
...
static void `$INSTANCE_NAME`_SendSequence(void) CYREENTRANT
{
    /* Send the command pattarn to LCD */
    ...
    {
        /* Wait until I2C Master will finish the transaction */
    }
...
*  uint8 dataLength: length of data to be send for this command.
                                                        ...
                                                        `=ReentrantKeil($INSTANCE_NAME . "_HandleCustomCommand")`
{
    ...
    {   
        if (cmdId == `$INSTANCE_NAME`_cmdPatt[cmdIndx][`$INSTANCE_NAME`_COMMAND_BYTE_INDEX])
        {
            ...
            {
                /* Get next byte from the array of data */
                ch = cmdData[i];

                /* Add current byte to the buffer */
...
void `$INSTANCE_NAME`_PrintString(char8 const string[]) `=ReentrantKeil($INSTANCE_NAME . "_PrintString")`
{
    ...
    /* Until null is reached, put next character into the buffer */
    while((char8) '\0' != current)
    {
        /* Add current byte to the buffer */
            ...
            * that. But the first eight generated character are exeptions.
            */
...
*
* Parameters:
*  character:  ASCII character to be displayed on the LCD module's screen.
*
* Return:
*  None
*
...
{
    /* Send one byte of data with `$INSTANCE_NAME`_WriteData() */
    #if (`$INSTANCE_NAME`_COMMAND_FORMAT == `$INSTANCE_NAME`_NXP_PCF2119X)
        /* In case of NXP PCF2119x, the LCD modules use character set "S"
        * (see LCD's module datasheet) that has english letters located in
        * "extended" area (ASCII code > 128) so it is required to handle
...
void `$INSTANCE_NAME`_SetAddr(uint8 address) `=ReentrantKeil($INSTANCE_NAME . "_SetAddr")`
{
    ...
    *******************************************************************************/
                                                            ...
                                                            `=ReentrantKeil($INSTANCE_NAME . "_LoadCustomFonts")`
    {
        ...
        #if (`$INSTANCE_NAME`_COMMAND_FORMAT == `$INSTANCE_NAME`_NXP_PCF2119X)
            /* Set starting address in the LCD Module as it is the location of CGRAM */
