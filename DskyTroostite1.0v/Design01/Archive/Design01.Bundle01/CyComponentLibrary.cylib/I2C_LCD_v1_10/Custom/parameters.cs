...
namespace I2C_LCD_v1_10
{
    ...
    public class CyParameters
    {
        ...
        public XmlSerializer m_requiredCommandsSerializer;
        public XmlSerializer m_customCommandsSerializer;
        ...
        // Custom commands tab parameters
        ...
        private void AssignDefaultValues(ref List<CyRequiredCommandRow> list)
        {
            ...
            if (defaultReqCmds.Count == list.Count)
            {
                ...
                {
                    list[cmdIndex].m_isDataSizeReadOnly = defaultReqCmds[cmdIndex].m_isDataSizeReadOnly;
                    list[cmdIndex].m_cmdDescription = defaultReqCmds[cmdIndex].m_cmdDescription;
                    list[cmdIndex].m_cmdName = defaultReqCmds[cmdIndex].m_cmdName;
                    if (list[cmdIndex].m_isDataSizeReadOnly)
                    {
                        list[cmdIndex].m_dataSize = defaultReqCmds[cmdIndex].m_dataSize;
                    }
                }
            }
            else
        ...
        public bool IsAPINameReserved(string name)
        {
            ...
            {
                if (RequiredCommands[i].m_cmdName.ToUpper() == name.ToUpper())
                {
                    isNameReserved = true;
                    break;
                }
        ...
        {
            try { return Convert.ToByte(((value != null) ? value.ToString() : string.Empty), 16); }
            catch { return null; }
        }
        ...
        public bool CheckCustomCommandsNullValues()
        {
            ...
            {
                if ((CustomCommands[i].m_cmdSize == null) ||
                    (CustomCommands[i].m_dataSize == null) ||
                    (CustomCommands[i].m_cmdByte1 == null) ||
                    (CustomCommands[i].m_cmdSize != null &&
                               CustomCommands[i].m_cmdSize > 1 && CustomCommands[i].m_cmdByte2 == null) ||
                    (CustomCommands[i].m_cmdSize != null &&
                               CustomCommands[i].m_cmdSize > 2 && CustomCommands[i].m_cmdByte3 == null) ||
                    (CustomCommands[i].m_cmdSize != null &&
                               CustomCommands[i].m_cmdSize > 3 && CustomCommands[i].m_cmdByte4 == null) ||
                    (CustomCommands[i].m_apiName == null))
                {
                    isOk = false;
                    break;
                }
        ...
        public bool CheckRequiredCommandsNullValues()
        {
            ...
            for (int i = 0; i < RequiredCommands.Count; i++)
            {
                if ((RequiredCommands[i].m_cmdByte1 == null) ||
                    (RequiredCommands[i].m_cmdSize != null &&
                                 RequiredCommands[i].m_cmdSize > 1 && RequiredCommands[i].m_cmdByte2 == null) ||
                    (RequiredCommands[i].m_cmdSize != null &&
                                 RequiredCommands[i].m_cmdSize > 2 && RequiredCommands[i].m_cmdByte3 == null) ||
                    (RequiredCommands[i].m_cmdSize != null &&
                                 RequiredCommands[i].m_cmdSize > 3 && RequiredCommands[i].m_cmdByte4 == null))
                {
                    isOk = false;
                    break;
                }
