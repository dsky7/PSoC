    ...
    partial class CyAPIGenerator
    {
                                    ...
                                    ref Dictionary<string, string> paramDict, int pNumCommonLines, int pNumSegmentLines)
        {
            ...
            using (TextWriter writerCVariables = new StringWriter(), writerHPixelDef = new StringWriter())
            {
                ...
                commons =
                    new StringBuilder(String.Format("static uint32 {0}_commons[{0}_COM_NUM] = {{{1}}};", m_instanceName,
                                                    commons));
                ...
                {
                    int comIndex = i;
                    writerHPixelDef.WriteLine(GenerateDefine(String.Format("COM{0}_PORT", comIndex), String.Format("{0}_Com__{1}__PORT",  m_instanceName, comIndex)));
                    writerHPixelDef.WriteLine(GenerateDefine(String.Format("COM{0}_PIN", comIndex), String.Format("{0}_Com__{1}__SHIFT",  m_instanceName, comIndex)));
                }
                writerHPixelDef.WriteLine("");
                ...
                {
                    int segIndex = i;
                    writerHPixelDef.WriteLine(GenerateDefine(String.Format("SEG{0}_PORT", segIndex), String.Format("{0}_Seg__{1}__PORT",  m_instanceName, segIndex)));
                    writerHPixelDef.WriteLine(GenerateDefine(String.Format("SEG{0}_PIN", segIndex), String.Format("{0}_Seg__{1}__SHIFT",  m_instanceName, segIndex)));
                }
                writerHPixelDef.WriteLine("");
        ...
        private void SetCharDotMatrixParam(Dictionary<string, string> paramDict)
        {
            using (TextWriter writerCharDotMatrix = new StringWriter())
            {
                ...
                StringBuilder dotMatrixParam =
                    new StringBuilder(string.Format("static const uint8 {0}_charDotMatrix[][5u] = ", m_instanceName));
            ...
            CyLCDParameters param)
        {
            ...
            for (int j = 0; j < 2; j++)
            {
                ...
                param.m_desiredLCDMode = (j == 0) ? CyLCDParameters.CyLCDModeType.LowSpeed :
                                                    CyLCDParameters.CyLCDModeType.HS;
                ...
                contrastDefines.AppendFormat("static const uint{0} {1}_dividers{2}S[2u][10u] = {{\r\n{{{3}}},\r\n{{{4}}}}};",
