...
static void `$INSTANCE_NAME`_WriteFrameBuffer(reg32 regAddr[], uint32 bitNumber, uint32 state)
{
    ...
    row = `$INSTANCE_NAME`_EXTRACT_ROW(bitNumber);
    port = `$INSTANCE_NAME`_EXTRACT_PORT(bitNumber);
    pin = `$INSTANCE_NAME`_EXTRACT_PIN(bitNumber);

    /* Write new bit value to the frame buffer. */
    mask = (uint32)(~((uint32)(`$INSTANCE_NAME`_PIXEL_STATE_ON << ((pin * `$INSTANCE_NAME`_MAX_BUFF_ROWS) + row))));

    regAddr[port] = (regAddr[port] & mask) | ((uint32)((state & `$INSTANCE_NAME`_PIXEL_STATE_ON) <<
                                                       ((pin * `$INSTANCE_NAME`_MAX_BUFF_ROWS) + row)));
}
...
static uint32 `$INSTANCE_NAME`_ReadFrameBuffer(const reg32 regAddr[], uint32 bitNumber)
{
    ...
    pixelState = ((uint32)(regAddr[port] >> ((pin * `$INSTANCE_NAME`_MAX_BUFF_ROWS) + row))) &
...
void `$INSTANCE_NAME`_Init(void)
{
    ...
    `$INSTANCE_NAME`_DIVIDER_REG = (`$INSTANCE_NAME`_DIVIDER_REG & ((uint32)(~`$INSTANCE_NAME`_DIVIDER_MASK)))
                                 | (`$INSTANCE_NAME`_SUBFR_DIVIDER | `$INSTANCE_NAME`_DEAD_DIVIDER);

    /* Set Configuration Register */
    `$INSTANCE_NAME`_CONTROL_REG = (`$INSTANCE_NAME`_CONTROL_REG &
      ((uint32)(~(`$INSTANCE_NAME`_MODE | `$INSTANCE_NAME`_TYPE | `$INSTANCE_NAME`_OP_MODE |
                  `$INSTANCE_NAME`_BIAS_MASK | `$INSTANCE_NAME`_COM_NUM_MASK))))
      | (`$INSTANCE_NAME`_LCD_SPEED_MODE | `$INSTANCE_NAME`_WAVEFORMS_TYPE | `$INSTANCE_NAME`_DRIVING_OP_MODE
      | `$INSTANCE_NAME`_PWM_BIAS | `$INSTANCE_NAME`_COM_NUMBER);

    /* Need to clear display to start normal work.
    *  `$INSTANCE_NAME`_ClearDisplay function will set Commons in frame buffer also.
    */
...
void `$INSTANCE_NAME`_WriteInvertState(uint32 invertState)
{
    ...
    if (invertState != `$INSTANCE_NAME`_invertState)
    {
        ...
        `$INSTANCE_NAME`_DATA0_REG = (uint32)(~`$INSTANCE_NAME`_DATA0_REG);
        `$INSTANCE_NAME`_DATA1_REG = (uint32)(~`$INSTANCE_NAME`_DATA1_REG);
        `$INSTANCE_NAME`_DATA2_REG = (uint32)(~`$INSTANCE_NAME`_DATA2_REG);
        `$INSTANCE_NAME`_DATA3_REG = (uint32)(~`$INSTANCE_NAME`_DATA3_REG);
        `$INSTANCE_NAME`_DATA4_REG = (uint32)(~`$INSTANCE_NAME`_DATA4_REG);
...
void `$INSTANCE_NAME`_ClearDisplay(void)
{
    ...
    `$INSTANCE_NAME`_DATA0_REG = (uint32)0x00000000u;
    `$INSTANCE_NAME`_DATA1_REG = (uint32)0x00000000u;
    `$INSTANCE_NAME`_DATA2_REG = (uint32)0x00000000u;
    `$INSTANCE_NAME`_DATA3_REG = (uint32)0x00000000u;
    `$INSTANCE_NAME`_DATA4_REG = (uint32)0x00000000u;
