...
*
* Return: 
*   cystatus: This function will return CYRET_SUCCESS if data is sent succesfully.
*
* Side Effects:
...
*
* Return: 
*  cystatus: This function will return CYRET_SUCCESS if atleast one byte is received
*            succesfully within the timeout interval .If no data is received  this 
         ...
         `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommRead")`
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    ...
    {
        /* If atleast one byte is received within the timeout interval enter the next loop
        * waiting for more data reception */
        if(0u != `$INSTANCE_NAME`_GetRxBufferSize()) 
        {
            ...
            * then it is considered as end of transmitted data block(packet) from the host and
            * the program execution will break from the data awaiting loop with status=CYRET_SUCCESS */
            do
            {
