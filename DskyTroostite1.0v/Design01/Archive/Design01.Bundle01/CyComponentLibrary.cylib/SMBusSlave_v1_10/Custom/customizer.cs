    ...
    public class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1, ICyDRCProvider_v1
    {
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            if (m_parameters.Mode == CyEModeSelType.PMBUS_SLAVE)
            {
                for (int i = 0; i < m_parameters.PmBusTable.Count; i++)
                {
                    ...
                    varStructure += GetStructureField(m_instName, m_parameters.PagedCommands,
                        ...
                        m_parameters.PmBusTable[i].m_paged, m_parameters.PmBusTable[i].m_enable, i, ref tempArray);
                }
            }
            for (int i = 0; i < m_parameters.CustomTable.Count; i++)
            {
                ...
                {
                    regStructure += GetStructureField(null, m_parameters.PagedCommands,
                        m_parameters.CustomTable[i].m_name, m_parameters.CustomTable[i].m_size,
                        m_parameters.CustomTable[i].m_paged, m_parameters.CustomTable[i].m_enable, i,
                        ref customGenerateAsArray);
            ...
            strNullStructure += TAB + "// FLASH_CRC" + NEW_LINE + TAB + "{0x0000u}" + NEW_LINE
                + "}";
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            if (m_parameters.CheckPmBusTableNullValues() == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error, string.Format(Resources.
                    DrcNullValuesError, Resources.PMBusCmdsTabDisplayName));
            }
