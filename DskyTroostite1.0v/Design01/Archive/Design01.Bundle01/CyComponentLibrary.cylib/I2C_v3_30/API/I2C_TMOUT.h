...
uint16 `$INSTANCE_NAME`_TimeoutGetPeriod(void)          `=ReentrantKeil($INSTANCE_NAME . "_TimeoutGetPeriod")`;
    ...
    #define `$INSTANCE_NAME`_TimeoutEnableInt()             do{ \
                                                                `$INSTANCE_NAME`_TMOUT_CSR_REG |=                  \
                                                                ((uint8) `$INSTANCE_NAME`_TMOUT_CSR_I2C_TMOUT_IE); \
                                                            }while(0)

    #define `$INSTANCE_NAME`_TimeoutDisableInt()            do{ \
                                                                `$INSTANCE_NAME`_TMOUT_CSR_REG &=                   \
                                                                ((uint8) ~`$INSTANCE_NAME`_TMOUT_CSR_I2C_TMOUT_IE); \
                                                            }while(0)
    ...
    #define `$INSTANCE_NAME`_TimeoutSetIntrMode(intrSource) do{ \
                                                                `$INSTANCE_NAME`_TMOUT_CSR_REG |= (intrSource &  \
    ...
    #define `$INSTANCE_NAME`_TimeoutSetIntrMode(intrSource) do{ \
                                                                `$INSTANCE_NAME`_TMOUT_INTR_MASK_REG = (intrSource &  \
                                                                               `$INSTANCE_NAME`_TMOUT_STS_INTR_MASK); \
                                                            }while(0)

    #define `$INSTANCE_NAME`_TimeoutGetIntrMode()       (`$INSTANCE_NAME`_TMOUT_INTR_MASK_REG & \
                                                         `$INSTANCE_NAME`_TMOUT_STS_INTR_MASK)

    #define `$INSTANCE_NAME`_TimeoutGetStatus()         (`$INSTANCE_NAME`_TMOUT_STS_REG & \
                                                                            `$INSTANCE_NAME`_TMOUT_STS_INTR_MASK)

    #define `$INSTANCE_NAME`_TimeoutClearStatus(intrStatus) /* There is no implementation for UDB version currently */
...
***************************************/
    ...
    /* Fixed hardware for timeout only avilable for PSoC5 LP */
    #if(CY_PSOC5LP)
    ...
    #define `$INSTANCE_NAME`_TMOUT_INTR_ENABLE_PTR ( (reg8 *) `$INSTANCE_NAME`_bTimeoutTimer_StsReg__STATUS_AUX_CTL_REG)
        ...
        #define `$INSTANCE_NAME`_TMOUT_SCL_AUX_CTRL_REG (* (reg8 *) \
                                                            `$INSTANCE_NAME`_bTimeoutTimer_Scl_dpScl_u0__DP_AUX_CTL_REG)
        #define `$INSTANCE_NAME`_TMOUT_SCL_AUX_CTRL_PTR (  (reg8 *) \
                                                            `$INSTANCE_NAME`_bTimeoutTimer_Scl_dpScl_u0__DP_AUX_CTL_REG)

        #if(`$INSTANCE_NAME`_TIMEOUT_PRESCALER_ENABLED)
            #define `$INSTANCE_NAME`_SCL_PRESCALER_AUX_CTRL_REG (* (reg8 *) \
                                                 `$INSTANCE_NAME`_bTimeoutTimer_SclPrescaler_prScl__CONTROL_AUX_CTL_REG)
            #define `$INSTANCE_NAME`_SCL_PRESCALER_AUX_CTRL_PTR (  (reg8 *) \
                                                 `$INSTANCE_NAME`_bTimeoutTimer_SclPrescaler_prScl__CONTROL_AUX_CTL_REG)

            #define `$INSTANCE_NAME`_SCL_PRESCALER_PRD_REG  (* (reg8 *) \
                                                          `$INSTANCE_NAME`_bTimeoutTimer_SclPrescaler_prScl__PERIOD_REG)
            #define `$INSTANCE_NAME`_SCL_PRESCALER_PRD_PTR  (  (reg8 *) \
                                                          `$INSTANCE_NAME`_bTimeoutTimer_SclPrescaler_prScl__PERIOD_REG)
        ...
        #define `$INSTANCE_NAME`_TMOUT_SDA_AUX_CTRL_REG (* (reg8 *) \
                                                            `$INSTANCE_NAME`_bTimeoutTimer_Sda_dpSda_u0__DP_AUX_CTL_REG)
        #define `$INSTANCE_NAME`_TMOUT_SDA_AUX_CTRL_PTR (  (reg8 *) \
                                                            `$INSTANCE_NAME`_bTimeoutTimer_Sda_dpSda_u0__DP_AUX_CTL_REG)

        #if(`$INSTANCE_NAME`_TIMEOUT_PRESCALER_ENABLED)
            #define `$INSTANCE_NAME`_SDA_PRESCALER_AUX_CTRL_REG (* (reg8 *) \
                                                 `$INSTANCE_NAME`_bTimeoutTimer_SdaPrescaler_prSda__CONTROL_AUX_CTL_REG)
            #define `$INSTANCE_NAME`_SDA_PRESCALER_AUX_CTRL_PTR (  (reg8 *) \
                                                 `$INSTANCE_NAME`_bTimeoutTimer_SdaPrescaler_prSda__CONTROL_AUX_CTL_REG)

            #define `$INSTANCE_NAME`_SDA_PRESCALER_PRD_REG  (* (reg8 *) \
                                                          `$INSTANCE_NAME`_bTimeoutTimer_SdaPrescaler_prSda__PERIOD_REG)
            #define `$INSTANCE_NAME`_SDA_PRESCALER_PRD_PTR  (  (reg8 *) \
                                                          `$INSTANCE_NAME`_bTimeoutTimer_SdaPrescaler_prSda__PERIOD_REG)
...
#if(`$INSTANCE_NAME`_TIMEOUT_FF_ENABLED)
    /* Returns true if timeuot functionality is enabled */
