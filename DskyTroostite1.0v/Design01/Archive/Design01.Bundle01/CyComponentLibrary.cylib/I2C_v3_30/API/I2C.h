...
typedef struct
{
    uint8 enableState;
        ...
        uint8  tmoutIntr;
            ...
            uint8 tmoutPrd;
        #endif /* (`$INSTANCE_NAME`_TIMEOUT_PRESCALER_ENABLED && CY_UDB_V0) */
...
#define `$INSTANCE_NAME`_FF_RESET_DELAY     (0x02u)
...
#define `$INSTANCE_NAME`_GET_SLAVE_ADDR(dataReg)   (((dataReg) >> `$INSTANCE_NAME`_SLAVE_ADDR_SHIFT) & \
    ...
    #define `$INSTANCE_NAME`_GENERATE_START         do{ \
        ...
        #define `$INSTANCE_NAME`_GENERATE_STOP      do{ \
                                                        `$INSTANCE_NAME`_CSR_REG = `$INSTANCE_NAME`_CSR_STOP_GEN; \
                                                    }while(0)
    ...
    #define `$INSTANCE_NAME`_MCSR_REG           (`$INSTANCE_NAME`_CSR_REG)   /* UDB incoporates master and slave regs */
    ...
    #define `$INSTANCE_NAME`_GENERATE_RESTART   do{ \
...
#define `$INSTANCE_NAME`_ENABLE     (1u)
    ...
    #define `$INSTANCE_NAME`_DEFAULT_CLK_RATE   ((CY_PSOC5A) ? (`$INSTANCE_NAME`_DEFAULT_CLK_RATE0) : \
                                                               (`$INSTANCE_NAME`_DEFAULT_CLK_RATE1))
