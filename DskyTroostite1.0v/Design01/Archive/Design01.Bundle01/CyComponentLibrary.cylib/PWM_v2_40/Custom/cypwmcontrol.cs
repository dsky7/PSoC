...
namespace PWM_v2_40
{
    public partial class CyPWMControl : UserControl
    {
        ...
        private int m_value5;
        private int m_value6;
        ...
        public void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            if (!PWMModeFound)
            {
                m_cbPWMMode.Text = prms.m_pwmMode.Expr;
            }

            if (prms.m_fixedFunction.Value == "true")
            ...
            try
            {
                m_numPeriod.Value = Convert.ToInt32(prms.m_period.Value);
            }

            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
                    + prms.m_period.Value.ToString());
            ...
            try
            {
                m_numCompare1.Value = Convert.ToInt32(prms.m_compareValue1.Value);
            }

            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
                    + prms.m_compareValue1.Value.ToString());
            ...
            try
            {
                m_numCompare2.Value = Convert.ToInt32(prms.m_compareValue2.Value);
            }

            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
                    + prms.m_compareValue2.Value.ToString());
            ...
            if (prms.m_compareType1.Value == Greater_Equal_Str)
            ...
            foreach (string str in DitherOffsetEnums)
            {
                if (!DithOffFound)
                {
                    string paramcompare = m_instEdit.ResolveEnumIdToDisplay(DITHEROFFSET, prms.m_ditherOffset.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!DithOffFound)
            {
                m_cbDitherOffset.Text = prms.m_ditherOffset.Expr;
            }
            ...
            foreach (string str in CT1Enums)
            {
                if (!CT1found)
                {
                    string paramcompare = m_instEdit.ResolveEnumIdToDisplay(COMPARETYPE1, prms.m_compareType1.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!CT1found)
            {
                m_cbCompareType1.Text = prms.m_compareType1.Expr;
            }
            ...
            foreach (string str in CT2Enums)
            {
                if (!CT2found)
                {
                    string paramcompare = m_instEdit.ResolveEnumIdToDisplay(COMPARETYPE2, prms.m_compareType2.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!CT2found)
            {
                m_cbCompareType2.Text = prms.m_compareType2.Expr;
            }
            ...
            foreach (string str in DBModeEnums)
            {
                if (!DBMfound)
                {
                    string paramcompare = m_instEdit.ResolveEnumIdToDisplay(DEADBAND, prms.m_deadBand.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!DBMfound)
            {
                m_cbDeadBandMode.Text = prms.m_deadBand.Expr;
            }

            Int32 DeadTimeValue = DeadTimeDefaultValue;
            try
            {
                DeadTimeValue = Convert.ToInt32(prms.m_deadTime.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
                    + prms.m_deadTime.Value.ToString());
        ...
        public void UpdateDrawing(CyPWMParameters prms)
        {
            ...
            if (prms.m_pwmMode.Value == Center_Align_Str)
            {
            ...
            if (prms.m_pwmMode.Value != Center_Align_Str)
            {
                ...
                SizeF pervalsize = wfg.MeasureString(prms.m_period.Value, perfont);
                ...
                wfg.DrawString(prms.m_period.Value, perfont, blkbrush, pervalrect1);

                RectangleF pervalrect2 = new RectangleF(startright + (2 * PB_POLYGON_WIDTH),
                    PB_EXTENTS_BORDER + PB_POLYGON_WIDTH - (pervalsize.Height / 2),
                    pervalsize.Width, pervalsize.Height);
                wfg.FillRectangle(whitebrush, pervalrect2);
                wfg.DrawString(prms.m_period.Value, perfont, blkbrush, pervalrect2);
            ...
            else
            {
                ...
                SizeF periodsize = wfg.MeasureString(prms.m_period.Value, perfont);
                ...
                wfg.DrawString(prms.m_period.Value, perfont, blkbrush, per1rect);

                RectangleF per2rect = new RectangleF(cacenter2 + (2 * PB_POLYGON_WIDTH),
                    PB_EXTENTS_BORDER + PB_POLYGON_WIDTH - (periodsize.Height / 2), periodsize.Width,
                    periodsize.Height);
                wfg.FillRectangle(whitebrush, per2rect);
                wfg.DrawString(prms.m_period.Value, perfont, blkbrush, per2rect);
            }
            ...
            if (prms.m_pwmMode.Value == One_Output_Str || (prms.m_pwmMode.Value == Center_Align_Str) ||
                (prms.m_pwmMode.Value == Dither_Str))
            {
                wfnames.Add(PWM);
            }
            else if (prms.m_pwmMode.Value == Two_Outputs_Str)
            {
                wfnames.Add(PWM1);
                wfnames.Add(PWM2);
            }
            else if (prms.m_pwmMode.Value == Dual_Edge_Str || (prms.m_pwmMode.Value == Hardware_Select_Str))
            {
                wfnames.Add(PWM1);
                wfnames.Add(PWM2);
                wfnames.Add(PWM);
            ...
            if (prms.m_deadBand.Value == "1" || prms.m_deadBand.Value == "2")
            {
                wfnames.Add(PH1);
                wfnames.Add(PH2);
            }
            ...
            try
            {
                PeriodValue32 = Convert.ToInt32(prms.m_period.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of UInt16 type "
            ...
            try
            {
                DeadTimeValue32 = Convert.ToInt32(prms.m_deadTime.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
            ...
            try
            {
                CompareValue32 = Convert.ToInt32(prms.m_compareValue1.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
            ...
            try
            {
                Compare2Value32 = Convert.ToInt32(prms.m_compareValue2.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
            ...
            try
            {
                Compare1Value16 = Convert.ToUInt16(prms.m_compareValue1.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
            ...
            try
            {
                Compare2Value16 = Convert.ToUInt16(prms.m_compareValue2.Value);
            }
            catch (OverflowException)
            {
                string error_str = String.Format("{0} is outside the range of Int32 type "
            ...
            for (int i = 0; i < numwaveforms; i++)
            {
                ...
                switch (wfnames[i])
                {
                    ...
                    case PWM1:
                        ...
                        if (prms.m_compareType1.Value == Less_Str || prms.m_compareType1.Value == Less_Equal_Str)
                        {
                            StartHigh = false;
                            comparestring1 = " <= " + prms.m_compareValue1.Value;

                            if (prms.m_compareType1.Value != Less_Equal_Str)
                            {
                                Edge1 += bitwidth;
                                Edge1_2 += bitwidth;
                                comparestring1 = " < " + prms.m_compareValue1.Value;
                           ...
                           ((CompareValue32 >= PeriodValue32) &&
                             prms.m_compareType1.Value == Less_Equal_Str))
                            {
                        ...
                        else if (prms.m_compareType1.Value == Greater_Str ||
                            prms.m_compareType1.Value == Greater_Equal_Str ||
                            prms.m_compareType1.Value == FirmwareControl_Str)
                        {
                            StartHigh = true;
                            comparestring1 = " > " + prms.m_compareValue1.Value;
                            if ((prms.m_compareType1.Value == Greater_Equal_Str) ||
                            ...
                            if (((CompareValue32 >= PeriodValue32) && prms.m_compareType1.Value == Greater_Str) ||
                              ((CompareValue32 > PeriodValue32) && ((prms.m_compareType1.Value == Greater_Equal_Str) ||
                              (prms.m_compareType1.Value == FirmwareControl_Str))))
                            {
                                pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                pts[1] = pts[0];
                        ...
                        // "Equal" compare type; cmp value > period value 
                                  ...
                                  && (prms.m_compareType1.Value == Equal_Str))
                        {
                    ...
                    case PWM2:
                        ...
                        if (prms.m_compareType2.Value == Less_Str || prms.m_compareType2.Value == Less_Equal_Str)
                        {
                            StartHigh = false;
                            comparestring1 = " <= " + prms.m_compareValue2.Value;
                            if (prms.m_compareType2.Value != Less_Equal_Str)
                            {
                                Edge1 += bitwidth;
                                Edge1_2 += bitwidth;
                                comparestring1 = " < " + prms.m_compareValue2.Value;
                            ...
                            if ((Compare2Value32 > PeriodValue32) ||
                               ((Compare2Value32 >= PeriodValue32) &&
                                 prms.m_compareType2.Value == Less_Equal_Str))
                            {
                                pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                pts[1] = pts[0];
                                pts[2] = pts[0];
                        ...
                        else if (prms.m_compareType2.Value == Greater_Str ||
                            prms.m_compareType2.Value == Greater_Equal_Str ||
                            prms.m_compareType2.Value == FirmwareControl_Str)
                        {
                            StartHigh = true;
                            comparestring1 = " > " + prms.m_compareValue2.Value;
                            if ((prms.m_compareType2.Value == Greater_Equal_Str) ||
                                (prms.m_compareType2.Value == FirmwareControl_Str))
                            {
                                Edge1 += bitwidth;
                                Edge1_2 += bitwidth;
                            ...
                            if (((Compare2Value32 >= PeriodValue32) && prms.m_compareType2.Value == Greater_Str) ||
                              ((Compare2Value32 > PeriodValue32) && ((prms.m_compareType2.Value == Greater_Equal_Str) ||
                              (prms.m_compareType2.Value == FirmwareControl_Str))))
                            {
                                pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                pts[1] = pts[0];
                            ...
                            else
                            {
                                ...
                                pts[17] = pts[7];
                                  ...
                                  && (prms.m_compareType2.Value == Equal_Str))
                        {
                            StartHigh = true;
                            Edge1 += bitwidth;
                            Edge1_2 += bitwidth;
                    ...
                    case PWM:
                        ...
                        #region PWM-ONE
                        if (prms.m_pwmMode.Value == One_Output_Str)
                        {
                            Edge1 = startleft + (bitwidth * (float)(PeriodValue32 - CompareValue32));
                            Edge1_2 = startright + (bitwidth * (float)(PeriodValue32 - CompareValue32));
                            if (prms.m_compareType1.Value == Less_Str || prms.m_compareType1.Value == Less_Equal_Str)
                                ...
                                prms.m_compareType1.Value == FirmwareControl_Str)
                            {
                                ...
                                if (((CompareValue32 >= PeriodValue32) &&
                                    prms.m_compareType1.Value == Greater_Str) || ((CompareValue32 > PeriodValue32)
                                    && ((prms.m_compareType1.Value == Greater_Equal_Str) ||
                                    (prms.m_compareType1.Value == FirmwareControl_Str))))
                                {
                                    pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                        ...
                        #endregion
                        #region PWM-DUAL
                        if (prms.m_pwmMode.Value == Dual_Edge_Str)
                        {
                            string cmptype1 = "0";
                            string cmptype2 = "0";
                            Edge1 = startleft + (bitwidth * (float)(PeriodValue32 - CompareValue32));
                            ...
                            switch (cmptype1 + cmptype2)
                            {
                                ...
                                case "XX": //cmp1type = "Equal", cmp2type = "Equal"
                                    ...
                                    {
                                        if (prms.m_compareValue1.Value == prms.m_compareValue2.Value)
                                        {
                                            pts[0] = new PointF(startleft, LowY);
                                            pts[1] = new PointF(Edge1, LowY);
                                            pts[2] = new PointF(Edge1, HighY);
                                ...
                                case "X0": //cmp1type = "Equal", cmp2type = "Less Than"
                                    ...
                                    {
                                        if ((prms.m_compareType2.Value == Equal_Str &&
                                ...
                                case "0X": //cmp1type = "Less Than", cmp2type = "Equal"
                                    ...
                                    {
                                        if ((prms.m_compareType1.Value == Equal_Str &&
                                            (Compare1Value16 >= Compare2Value16)) ||
                                            (Compare1Value16 > Compare2Value16))
                                        {
                                            pts[0] = new PointF(startleft, LowY);
                        ...
                        if (prms.m_pwmMode.Value == Center_Align_Str)
                        {
                            ...
                            if (prms.m_compareType1.Value == Less_Str || prms.m_compareType1.Value == Less_Equal_Str)
                            {
                                ...
                                if (prms.m_compareType1.Value == Less_Equal_Str)
                                {
                                    ...
                                    if (prms.m_compareValue1.Value == prms.m_period.Value)
                                    {
                                        Edge1 -= (bitwidth / 2f);
                                        Edge1_2 -= (bitwidth / 2f);
                                        Edge2 += (bitwidth / 2f);
                                ...
                                else
                                {
                                    ...
                                    if (prms.m_compareValue1.Value == "0")
                                    {
                                        Edge1 += (bitwidth / 2f);
                                        Edge1_2 += (bitwidth / 2f);
                                        Edge2 -= (bitwidth / 2f);
                            ...
                            else
                            {
                                ...
                                else if (prms.m_compareValue1.Value == "0")
                                {
                                    Edge1 = startleft;
                        ...
                        #endregion
                        #region PWM-HARDWARE
                        if (prms.m_pwmMode.Value == Hardware_Select_Str)
                        {
                        ...
                        #endregion
                        #region PWM-DITHER
                        if (prms.m_pwmMode.Value == Dither_Str)
                        {
                            ...
                            StartHigh = true;
                            string tmp = ".0";
                            switch (prms.m_ditherOffset.Value)
                            {
                                case "0": tmp = ".00"; break;
                                case "1": tmp = ".25"; break;
                                case "2": tmp = ".50"; break;
                            ...
                            if (Convert.ToInt32(prms.m_compareType1.Value) > 3) // Left Aligned 
                            {
                                StartHigh = true;
                                comparestring1 = "> " + prms.m_compareValue1.Value + tmp;
                            }
                            else
                            {
                                StartHigh = false;
        ...
        {
            UpdateDrawing(new CyPWMParameters(m_instEdit));
        }
        #endregion
        ...
        void m_numCompare2_DownEvent(object sender, CyDownButtonEventArgs e)
        {
            if (m_numCompare2.Value == PERIODMIN)
                e.m_cancel = true;
        }

        void m_numCompare2_UpEvent(object sender, CyUpButtonEventArgs e)
        {
        ...
        void m_numDeadBandCounts_DownEvent(object sender, CyDownButtonEventArgs e)
        {
            if (m_cbFFDeadBandMode.Visible)
            {
        ...
        void m_numDeadBandCounts_UpEvent(object sender, CyUpButtonEventArgs e)
        {
            if (m_cbFFDeadBandMode.Visible)
            {
        ...
        {
            m_value1 = m_control_advanced.m_cbEnableMode.SelectedIndex;
            m_value2 = m_control_advanced.m_cbRunMode.SelectedIndex;
            m_value3 = m_control_advanced.m_cbTriggerMode.SelectedIndex;
            m_value4 = m_control_advanced.m_cbKillMode.SelectedIndex;
            m_value5 = m_control_advanced.m_cbCaptureMode.SelectedIndex;
            m_value6 = m_cbPWMMode.SelectedIndex;
        ...
        {
            m_control_advanced.m_cbEnableMode.SelectedIndex = m_value1;
            m_control_advanced.m_cbRunMode.SelectedIndex = m_value2;
            m_control_advanced.m_cbTriggerMode.SelectedIndex = m_value3;
            m_control_advanced.m_cbKillMode.SelectedIndex = m_value4;
            m_control_advanced.m_cbCaptureMode.SelectedIndex = m_value5;
            m_cbPWMMode.SelectedIndex = m_value6;
            if (!m_rbUDB.Checked)
            {
                m_rbFixedFunction.Checked = false;
                m_rbUDB.Checked = true;
        ...
        {
            if (m_instEdit.DeviceQuery.IsPSoC4)
            {
                ...
                m_control_advanced.UpdateFormFromParams(m_instEdit);
            }
        }
    }
