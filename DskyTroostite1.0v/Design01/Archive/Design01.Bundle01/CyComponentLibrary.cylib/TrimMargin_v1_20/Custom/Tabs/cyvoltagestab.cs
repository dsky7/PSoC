...
namespace TrimMargin_v1_20
{
    public partial class CyVoltagesTab : CyEditingWrapperControl
    {
            ...
            : base(param)
        {
            ...
            CyParameters.VoltagesHeader_v2_0 = CyParameters.GetColNames(dgvVoltages);
        ...
        void numNumConverters_TextChanged(object sender, EventArgs e)
        {
            ...
            if (allowAddingRows)
            {
                if (m_params.NumConverters >= CyParamRanges.NUM_CONVERTERS_MIN &&
                    m_params.NumConverters <= CyParamRanges.NUM_CONVERTERS_MAX)
                {
                    bool newRowsAdded = false;
                    for (int i = m_params.VoltagesTable.Count; i < m_params.NumConverters; i++)
                    {
                        m_params.VoltagesTable.Add(CyVoltagesTableRow.CreateDefaultRow());
                        newRowsAdded = true;
                    }
                    for (int i = m_params.HardwareTable.Count; i < m_params.NumConverters; i++)
                    {
        ...
        private void dgvVoltages_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (m_params.GlobalEditMode)
            {
                ...
                if (e.ColumnIndex == colNominalOutputVoltage.Index)
                {
        ...
        private bool RangeCheck(int rowIndex, int colIndex)
        {
            ...
            if (colIndex == m_colVStartup.Index)
            {
                message = Resources.StartupVoltageError;
                currCellValue = m_params.VoltagesTable[rowIndex].m_startupVoltage;
            }
        ...
        private bool SanityCheck(int rowIndex)
        {
            ...
            try
            {
                ...
                double? startupVoltage = m_params.VoltagesTable[rowIndex].m_startupVoltage;
                ...
                // Vmin[x] <= VStartup[x] <= VMax[x]
                DataGridViewCell vStartupCell = dgvVoltages.Rows[rowIndex].Cells[m_colVStartup.Index];
                vStartupCell.ErrorText = CyParameters.ValueNotGreaterThan(minVoltage, startupVoltage,
                                          Resources.MinVoltageToVStartupSanityCheck);
                if (String.IsNullOrEmpty(vStartupCell.ErrorText))
                {
                    vStartupCell.ErrorText = CyParameters.ValueNotGreaterThan(startupVoltage, maxVoltage,
                                              Resources.VStartupToMaxVoltageSanityCheck);
                }
            }
            catch
            {
        ...
        public void UpdateUIFromTable()
        {
            ...
            for (int i = 0; i < m_dataGridRowCount; i++)
            {
                dgvVoltages.Rows.Add(new object[] {
                    ...
                    m_params.GetCalculatedVMin(i),
                    m_params.VoltagesTable[i].m_maxVoltage,
                    m_params.GetCalculatedVMax(i),
                    m_params.VoltagesTable[i].m_startupVoltage,
        ...
        public void UpdateTableFromUI()
        {
            ...
            for (int i = 0; i < m_dataGridRowCount; i++)
            {
                ...
                m_params.VoltagesTable[i].m_startupVoltage = CyParameters.ParseNullableDouble(
                    CyParameters.CellToString(dgvVoltages.Rows[i].Cells[m_colVStartup.Name].Value));
        ...
        public void UpdateCalculatedValues(int rowIndex)
        {
            dgvVoltages[m_colCalcMinVoltage.Index, rowIndex].Value = m_params.GetCalculatedVMin(rowIndex);
            dgvVoltages[m_colCalcMaxVoltage.Index, rowIndex].Value = m_params.GetCalculatedVMax(rowIndex);
        }
    }
}
