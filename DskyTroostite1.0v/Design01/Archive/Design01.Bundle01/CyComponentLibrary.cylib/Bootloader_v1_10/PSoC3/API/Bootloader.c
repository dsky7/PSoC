...
/* Our definition of a row size. */
    ...
    #define `$INSTANCE_NAME`_BL_LAST_ROW            (`$INSTANCE_NAME`_GET_REG16(`$INSTANCE_NAME`_MD_LAST_BLDR_ROW_ADDR  ))
    #define `$INSTANCE_NAME`_BL_APP_RUN_ADDR        (CY_GET_XTND_REG8 (`$INSTANCE_NAME`_MD_APP_RUN_ADDR        ))
    #define `$INSTANCE_NAME`_BL_APP_ENTRY_ADDR      (`$INSTANCE_NAME`_GET_REG32(`$INSTANCE_NAME`_MD_APP_ENTRY_POINT_ADDR))
    #define `$INSTANCE_NAME`_BL_APP_BYTE_LEN        (`$INSTANCE_NAME`_GET_REG32(`$INSTANCE_NAME`_MD_APP_BYTE_LEN        ))
...
*     Function Prototypes
***************************************/
cystatus `$INSTANCE_NAME`_WritePacket(uint8 command, uint8 CYXDATA* buffer, uint16 size) CYSMALL `=ReentrantKeil("`$INSTANCE_NAME`_WritePacket")`;
...
void `$INSTANCE_NAME`_HostLink(uint8 timeOut) `=ReentrantKeil("`$INSTANCE_NAME`_HostLink")`
{
    ...
    do
    {
        ...
        else
        {
            ...
            #else
                pktSize     = `$INSTANCE_NAME`_GET_REG16(&packetBuffer[`$INSTANCE_NAME`_SIZE_ADDR]);
                pktChecksum = `$INSTANCE_NAME`_GET_REG16(&packetBuffer[`$INSTANCE_NAME`_CHK_ADDR(pktSize)]);
            #endif  /* (!CY_PSOC4) */
                ...
                case `$INSTANCE_NAME`_COMMAND_REPORT_SIZE:
                    ...
                    if(communicationActive && (pktSize == 1u))
                    {
                        if(btldrData < `$INSTANCE_NAME`_FLASH_NUMBER_SECTORS)
                        {
                            ...
                            packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 2] = LO8(CY_FLASH_NUMBER_ROWS - 1);
                            packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 3] = HI8(CY_FLASH_NUMBER_ROWS - 1);
                        }
            ...
            ***************************************************************************/
            case `$INSTANCE_NAME`_COMMAND_PROGRAM:
                ...
                if(communicationActive && (pktSize >= 3u))
                {
                    ...
                    {
                        #if(!CY_PSOC4)
                            dataOffset = CY_GET_REG16(&(packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 1u]));
                        #else
                            dataOffset = `$INSTANCE_NAME`_GET_REG16(&(packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 1u]));
                        #endif  /* (!CY_PSOC4) */
                            ...
                            if(`$INSTANCE_NAME`_activeApp < 2u)
                            {
                                    ...
                                    uint16 firstRow = `$INSTANCE_NAME`_GET_REG32(\
            ...
            ***************************************************************************/
            case `$INSTANCE_NAME`_COMMAND_VERIFY:
                ...
                if(communicationActive && (pktSize == 3u))
                {
                    ...
                    #if(!CY_PSOC4)
                        uint16 CYDATA rowNum = CY_GET_REG16(&(packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 1u]));
                    #else
                        uint16 CYDATA rowNum =
                                        `$INSTANCE_NAME`_GET_REG16(&(packetBuffer[`$INSTANCE_NAME`_DATA_ADDR + 1u]));
                    #endif  /* (!CY_PSOC4) */
...
*
*******************************************************************************/
cystatus `$INSTANCE_NAME`_WritePacket(uint8 status, uint8 CYXDATA* buffer, uint16 size) CYSMALL `=ReentrantKeil("`$INSTANCE_NAME`_WritePacket")`
{
