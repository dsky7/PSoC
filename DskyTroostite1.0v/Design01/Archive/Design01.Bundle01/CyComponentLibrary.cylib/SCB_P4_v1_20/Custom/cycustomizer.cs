...
{
    public class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1, ICyDRCProvider_v1, ICyBootLoaderSupport, 
                                ICyExprEval_v2
    {
            ...
            ICyExpressMgr_v1 mgr)
        {
            ...
            // Open customizer with second page being active if mode != UNCONFIG and mode is correct for current device
            if (parameters.SCBMode != CyESCBMode.UNCONFIG)
            {
                List<CyCustErr> errList = new List<CyCustErr>(generalTab.GetErrors());
                bool generalPageHasErrors = (errList.Count > 0);
                
                List<string> visiblePageList = parameters.GetVisiblePageList(parameters.SCBMode);
                if ((visiblePageList.Count > 0) && (generalPageHasErrors == false))
                {
                    editor.SetSelectedPage(visiblePageList[0]);
                }
            }
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            // Validate SCB IP V1 limitations 
            errorList.AddRange(parameters.VerifySFSettings());
