...
namespace SCB_P4_v1_20
{
    public partial class CyI2CTab : CyTabControlWrapper
    {
        ...
        private void UpdateVisibility()
        {
            ...
            {
                if (m_chbEnableWakeup.Checked) //EnableWakeup is true
                {
                    if (m_chbEnableMedianFilter.Checked)
                    {
                        m_chbEnableMedianFilter.Enabled = true;
                        m_errorProvider.SetError(m_chbEnableMedianFilter, String.Format(Resources.ErrMsgMedianFilter));
                    }
                    else
                    {
                        m_chbEnableMedianFilter.Enabled = false;
                        m_errorProvider.SetError(m_chbEnableMedianFilter, String.Empty);
                    }
                }
                else //EnableWakeup is false
                {
                    m_chbEnableMedianFilter.Enabled = true;
                    m_errorProvider.SetError(m_chbEnableMedianFilter, String.Empty);
                }
        ...
        {
            m_binaryGridSlaveAddress.SetErrorListItem(6, (m_params.I2C_EnableWakeup && !m_params.IsScbIPV1) ? 
                CyBinaryGrid.OneSign : string.Empty);
            m_binaryGridSlaveAddressMask.SetErrorListItem(6, (m_params.I2C_EnableWakeup && !m_params.IsScbIPV1) ?
