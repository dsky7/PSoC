...
namespace VoltageFaultDetector_v2_30
{
    public partial class CyVoltagesTab : CyEditingWrapperControl
    {
        ...
        private bool SanityCheck(int rowIndex)
        {
            ...
            {
                // List of cell errors
                // Key: column index; Value: List of error messages
                Dictionary<int, List<string>> columnErrors = new Dictionary<int, List<string>>();
                for (int i = 0; i < dgvVoltages.ColumnCount; i++)
                {
                    columnErrors.Add(i, new List<string>());
                }
                ...
                #region Check1a: UVFault[x] <= VNom[x] <= OVFault[x]
                {
                    ...
                    if (nominalVoltage.HasValue)
                    {
                        ...
                        {
                            columnErrors[colNominalVoltage.Index].Add(message);
                        }
                    }
                }
                #endregion Check1a
                ...
                #region Check1b: UVFault[x] <= OVFault[x]
                {
                    ...
                    if (!nominalVoltage.HasValue && uvFaultThreshold.HasValue && ovFaultThreshold.HasValue)
                    {
                        ...
                        {
                            columnErrors[colUVFaultThreshold.Index].Add(message);
                        }
                    }
                }
                #endregion Check1b
                ...
                #region Check2: OVFault[x] multipled by Scale[x] is <= DACRange when ExtRef==False
                {
                    ...
                    // Validation is done on OVFault column. If it's disabled, then UVFault is validated instead.
                    double? compareValue = ovFaultThreshold;
                    int compareColumnIndex = colOVFaultThreshold.Index;
                    if (IsColumnDisabled(colOVFaultThreshold.Name))
                    {
                        compareValue = uvFaultThreshold;
                        compareColumnIndex = colUVFaultThreshold.Index;
                        message = message.Replace("OV", "UV");
                    }

                    if ((m_params.ExternalRef == false) && scale.HasValue && compareValue.HasValue &&
                        ((compareValue * scale) > dacRange + eps))
                    {
                        columnErrors[compareColumnIndex].Add(message);
                    }
                }
                #endregion Check2
                ...
                {
                    const double LIMIT_VOLTAGE = 0.05;
                    message = Resources.UvFaultMultScaleSanityCheck;
                    // Validation is done on UVFault column. If it's disabled, then OVFault is validated instead.
                    double? compareValue = uvFaultThreshold;
                    int compareColumnIndex = colUVFaultThreshold.Index;
                    if (IsColumnDisabled(colUVFaultThreshold.Name))
                    {
                        compareValue = ovFaultThreshold;
                        compareColumnIndex = colOVFaultThreshold.Index;
                        message = message.Replace("UV", "OV");
                    }

                    if (compareValue.HasValue && scale.HasValue && ((compareValue * scale) < LIMIT_VOLTAGE - eps))
                    {
                        columnErrors[compareColumnIndex].Add(message);
                    }
                }
                ...
                // Put error messages into cells
                foreach (KeyValuePair<int, List<string>> item in columnErrors)
                {
                    if (item.Value.Count > 0)
                    {
                        dgvVoltages[item.Key, rowIndex].ErrorText = item.Value[0];
                        noErrors = false;
                    }
                    else
                    {
                        dgvVoltages[item.Key, rowIndex].ErrorText = "";
                    }
                }
            }
            catch
