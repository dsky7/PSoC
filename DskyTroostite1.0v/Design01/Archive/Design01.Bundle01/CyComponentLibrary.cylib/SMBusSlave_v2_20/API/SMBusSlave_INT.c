...
*      Internal functions
**********************************/
static void `$INSTANCE_NAME`_ValidateCmdSize(void);
static void `$INSTANCE_NAME`_HandleLenError(void);
...
volatile uint8 `$INSTANCE_NAME`_isCmdPhase = 0u;          /* Indicates if command receiving phase active */
volatile uint8 `$INSTANCE_NAME`_cmdReceived = 0u;         /* Indicates if a command was received in this transaction */
volatile uint8 `$INSTANCE_NAME`_isValidCmd = 0u;          /* A flag indicates that a command is supported */

/* Intermediate buffer, where received data is stored until the end of
* write transaction. Or where the data is copied into when Read transaction
* begins.
*/
...
*      Internal variables
**********************************/
static uint8 `$INSTANCE_NAME`_isValidLen;
...
extern volatile uint8 `$INSTANCE_NAME`_cmdPage;            /* Current page value */
...
extern volatile uint8  `$INSTANCE_NAME`_curCmdType;
...
{
    static uint8 tmpByte;    /* Temporary store of I2C received byte */
    static uint8 tmpCsr;     /* Temporary store of I2C Control Status Register */
    ...
    */
    if(0u != (tmpCsr & `$INSTANCE_NAME`_I2C_CSR_ADDRESS))
    {
        ...
        */
        ...
        {
            `$INSTANCE_NAME`_ValidateCmdSize();
            
            /* This will provide a suport of incomplete block writes  */
            if(`$INSTANCE_NAME`_YES == `$INSTANCE_NAME`_isValidLen)
            {
                `$INSTANCE_NAME`_WriteHandler();            /* Process data received during "Write" transaction */
            }
            else
            {   
                `$INSTANCE_NAME`_HandleLenError();          /* Handle data mismatch */
            }
            ...
            /* Check for software address detection */
                    ...
                    if(0u != (`$INSTANCE_NAME`_I2C_DATA_REG & `$INSTANCE_NAME`_I2C_READ_FLAG))
                    {
                            ...
                            ***************************************/
                            ...
                            if(`$INSTANCE_NAME`_COMMAND_RECEIVED ==`$INSTANCE_NAME`_cmdReceived)
                            {
                                ...
                                {
                                    /* The "All Pages" wildcard is not valid for reading and should be 
                                    * reported as error. But is should be done only for paged commands.
                                    */
                                    ...
                                    {
                                        /* Protocol error occured - Invalid PAGE */
                                        `$INSTANCE_NAME`_HandleBusError(`$INSTANCE_NAME`_ERR_INVALID_DATA);
                                    }
                                }
                                else
                                {
                                    /* Reset I2C buffer to avoid erorneus read from it */
                                    ...
                                    /* Master attempts to read to many bytes. This happens when Read config.
                                    * set to "None".
                                    */
                            ...
                            ***************************************/
                            ...
                            else
                            {
                                    ...
                                    * should be serviced as in other cases `$INSTANCE_NAME`_cmdReceived should
                                    * always be true.
                                    */
                                    /* User code should provide a byte value for this transaction */
                ...
                */
                else if(tmpByte == `$INSTANCE_NAME`_alertResponseAddress)
                {
                        ...
                        /* Do nothing if AMBALERT pin isn't connected */
                        #if !defined(`$INSTANCE_NAME`_AlertSyncCtrlReg_Sync_ctrl_reg__REMOVED)
                        ...
                        #endif /* !defined(`$INSTANCE_NAME`_AlertSyncCtrlReg_Sync_ctrl_reg__REMOVED) */
    ...
    else if(`$INSTANCE_NAME`_I2C_CHECK_BYTE_COMPLETE(tmpCsr))
    {
        ...
        if(`$INSTANCE_NAME`_I2C_state == `$INSTANCE_NAME`_I2C_SM_SL_WR_DATA)
        {
            ...
            /* Command phase of a transaction */
...
*
* Summary:
*  Handles bus timeout occurrence event.
*
* Parameters:
*  void
*
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_ValidateCmdSize
********************************************************************************
*
* Summary:
*  Validates if the length of received command is valid. The validation is dobe
*  based on the type of command (either block or non-block). The 
*  `$INSTANCE_NAME`_isValidLen contains the results the validation. The valid
*  length means that for non block commands number of received data bytes is 
*  equal to size that is specified in customizer. For the block commands the
*  number of received data bytes shuld be equal to Byte Count plus one.
*
* Parameters:
*  None
*
...
static void `$INSTANCE_NAME`_ValidateCmdSize(void)
{
    `$INSTANCE_NAME`_isValidLen = `$INSTANCE_NAME`_NO;
    
    /* Check length for different types of commands */
    if(`$INSTANCE_NAME`_BLOCK_CMD == `$INSTANCE_NAME`_curCmdType)
    {
        /* In case of block type need to check if Byte Count field + 1 equals to buffer
        * index.
        */
        if((`$INSTANCE_NAME`_buffer[`$INSTANCE_NAME`_BLOCK_BYTE_COUNT_IDX] + 1u) == `$INSTANCE_NAME`_bufferIndex)
        {
            `$INSTANCE_NAME`_isValidLen = `$INSTANCE_NAME`_YES;
        }
    }
    else
    {
        /* For Send Byte and Read/Write Byte it is requred to check if buffer size
        * equals to buffer index.
        */
        if(`$INSTANCE_NAME`_bufferIndex == `$INSTANCE_NAME`_bufferSize)
        {
            `$INSTANCE_NAME`_isValidLen = `$INSTANCE_NAME`_YES;
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_HandleLenError
********************************************************************************
*
* Summary:
*  Analyzes the type of command and size of transaction and calls  
*  `$INSTANCE_NAME`_HandleBusError().
*
* Parameters:
*  None
*
...
{
    if((`$INSTANCE_NAME`_BLOCK_CMD == `$INSTANCE_NAME`_curCmdType) && 
    ((`$INSTANCE_NAME`_buffer[`$INSTANCE_NAME`_BLOCK_BYTE_COUNT_IDX] + 1u) > `$INSTANCE_NAME`_bufferIndex))
    {
        /* Report that host writes to few bytes and ignore command */
        `$INSTANCE_NAME`_HandleBusError(`$INSTANCE_NAME`_ERR_WR_TO_FEW_BYTES);
    }
    else if((`$INSTANCE_NAME`_NON_BLOCK_CMD == `$INSTANCE_NAME`_curCmdType) && 
    (`$INSTANCE_NAME`_bufferIndex < `$INSTANCE_NAME`_bufferSize))
    {
        /* Report that host writes to few bytes and ignore command */
        `$INSTANCE_NAME`_HandleBusError(`$INSTANCE_NAME`_ERR_WR_TO_FEW_BYTES);
    }
