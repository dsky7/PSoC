    ...
    public partial class CyConfigurationTab : CyTabControlWrapper
    {
        ...
        void m_cb_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (cb == m_cbInputRange)
            {
                ...
                if (value == CyEInputRange.Vssa_to_Vdda)
                {
                    ...
                    {
                        m_cbRef.Text = CyParameters.GetDescription(CyEReference.Internal_Vdda_3_Ref);
                    }
                    else
                    {
                        m_cbRef.Text = CyParameters.GetDescription(CyEReference.Internal_Vdda_4_Ref);
                    }                    
                }
                m_cbRef.Enabled = (value != CyEInputRange.Vssa_to_Vdda);
                UpdateClockRelatedFields(true);
            }
        ...
        void m_num_TextChanged(object sender, EventArgs e)
        {
            ...
            if (sender == m_numConvRate)
            {
                ...
                //else
                //{
                //    m_errorProvider.SetError(num, string.Format(Resource.ResolutionValueErrorDescription,
                //        CyParamRanges.MIN_RESOLUTION, CyParamRanges.MAX_RESOLUTION));
                //}
            }
            else if (sender == m_numRefValue)
            {
                ...
                {
                    m_errorProvider.SetError(num, string.Format(Resource.VrefValueErrorDescription,
                        CyParamRanges.MIN_VREF_VALUE, CyParamRanges.MAX_VREF_VALUE));
                }
            }
        }
        ...
        private bool UpdateClockRelatedFields(bool adjustSampleRate)
        {
            ...
            {
                UInt16 clockDivider = m_params.GetClockDivider((float)masterClockFrequency, frequency,
                                                               (float)minFreq, (float)maxFreq, m_configId);
