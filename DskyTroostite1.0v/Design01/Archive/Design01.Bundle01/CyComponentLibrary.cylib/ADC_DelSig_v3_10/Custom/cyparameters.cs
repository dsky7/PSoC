...
namespace ADC_DelSig_v3_10
{
    ...
    public class CyParameters
    {
        ...
        public UInt16 GetClockDivider(float masterClockFreq, float clockFreq, float minFreq, float maxFreq, 
                                      byte configId)
        {
            // Check arguments
            if ((masterClockFreq < 1) || (clockFreq < 1) || (masterClockFreq < clockFreq))
                return 1;

            UInt16 divider = (UInt16)Math.Round(masterClockFreq / clockFreq);
