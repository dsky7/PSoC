...
CY_ISR(`$INSTANCE_NAME`_EP_0_ISR)
{
    ...
    if ((bRegTemp & `$INSTANCE_NAME`_MODE_ACKD) != 0u)
    {
        modifyReg = 1u;
        if ((bRegTemp & `$INSTANCE_NAME`_MODE_SETUP_RCVD) != 0u)
        {
        ...
        else if ((bRegTemp & `$INSTANCE_NAME`_MODE_IN_RCVD) != 0u)
        {
            `$INSTANCE_NAME`_HandleIN();
        }
        else if ((bRegTemp & `$INSTANCE_NAME`_MODE_OUT_RCVD) != 0u)
        {
        ...
        if(modifyReg != 0u)
        {
            ...
            if((bRegTemp & `$INSTANCE_NAME`_MODE_SETUP_RCVD) == 0u)  /* Check if SETUP bit is not set, otherwise exit */
            {
                /* Update the count register */
                bRegTemp = `$INSTANCE_NAME`_ep0Toggle | `$INSTANCE_NAME`_ep0Count;
                CY_SET_REG8(`$INSTANCE_NAME`_EP0_CNT_PTR, bRegTemp);
                if(bRegTemp == CY_GET_REG8(`$INSTANCE_NAME`_EP0_CNT_PTR))   /* continue if writing was successful */
                {
                    do
                    {
...
void `$INSTANCE_NAME`_LoadEP0(void) `=ReentrantKeil($INSTANCE_NAME . "_LoadEP0")`
{
    ...
    {
        CY_SET_REG8((reg8 *)(`$INSTANCE_NAME`_EP0_DR0_IND + ep0Count), *`$INSTANCE_NAME`_currentTD.pData);
    ...
    `$INSTANCE_NAME`_lastPacketSize = ep0Count;
    `$INSTANCE_NAME`_ep0Count = ep0Count;
}
...
void `$INSTANCE_NAME`_ControlWriteDataStage(void) `=ReentrantKeil($INSTANCE_NAME . "_ControlWriteDataStage")`
{
    ...
    ep0Count = (CY_GET_REG8(`$INSTANCE_NAME`_EP0_CNT_PTR) & `$INSTANCE_NAME`_EPX_CNT0_MASK) -
               `$INSTANCE_NAME`_EPX_CNTX_CRC_COUNT;

    `$INSTANCE_NAME`_transferByteCount += ep0Count;

    while ((`$INSTANCE_NAME`_currentTD.count > 0u) && (ep0Count > 0u))
    {
        *`$INSTANCE_NAME`_currentTD.pData = CY_GET_REG8((reg8 *)(`$INSTANCE_NAME`_EP0_DR0_IND + regIndex));
        `$INSTANCE_NAME`_currentTD.pData = &`$INSTANCE_NAME`_currentTD.pData[1u];
        regIndex++;
        ep0Count--;
        `$INSTANCE_NAME`_currentTD.count--;
