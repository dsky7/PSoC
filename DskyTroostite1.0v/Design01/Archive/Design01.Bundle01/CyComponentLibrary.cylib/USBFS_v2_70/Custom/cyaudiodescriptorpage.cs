...
namespace USBFS_v2_70
{
    public partial class CyAudioDescriptorPage : CyBaseDescriptorPage
    {
        ...
        {
            get { return CyCustomizer.PAPAM_TAB_NAME_AUDIO; }
        }

        public override IEnumerable<CyCustErr> GetErrors()
        {
            // Save all changes before closing the customizer
            m_parameters.SerializedAudioDesc = m_parameters.SerializeDescriptorTree(m_parameters.m_audioTree);
        ...
        protected override bool IncludeASEPListItems(CyDescriptorNode interfaceNode, CyDescriptorNode selectedNode)
        {
            ...
            if (interfaceNode.m_value is CyAudioInterfaceDescriptor)
            {
                if (((CyAudioInterfaceDescriptor)interfaceNode.m_value).bInterfaceSubClass ==
                    (byte)CyUSBOtherTypes.CyAudioSubclassCodes.AUDIOSTREAMING)
                {
                    if ((selectedNode != null) && (selectedNode.m_value is CyAudioEndpointDescriptor))
                        includeASEP = true;
                }
            }
            return includeASEP;
