...
namespace CyCustomizer.CyStatusReg_v1_70
{
    public partial class CyBitfieldControl : UserControl
    {
        ...
        public const int DGV_HEIGHT = 199;
        public const int DGV_WIDTH = 236;
        public const int DGV_ROW_HEIGHT = 22;
        public const int DGV_THIRD_COL_WIDTH = 83;
        ...
        private void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            int.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_70.StatusRegParameters.MASKVALUE).Value,
                            out maskval);
            ...
            bool.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_70.StatusRegParameters.BUS).Value, out busValue);
            chBus.Checked = busValue;

            // Set Generate Interrupt check box
            bool.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_70.StatusRegParameters.INTRR).Value,
                            out intValue);
        ...
        private int GetNumInputs()
        {
            ...
            if (!Int32.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_70.StatusRegParameters.NUMINPUTS).Value,
                out inputs))
            {
        ...
        private void SetBitsVisibility(ICyInstEdit_v1 inst)
        {
            ...
            if (!(chInterrupt.Checked))
            {
                ...
                m_dgvBitFieldConfig.Width = DGV_WIDTH - DGV_THIRD_COL_WIDTH;
            }
            else
            {
                m_dgvBitFieldConfig.Columns[DGV_THIRD_COL].Visible = true;
                m_dgvBitFieldConfig.Width = DGV_WIDTH;
            }

            m_dgvBitFieldConfig.Height = DGV_HEIGHT - (DGV_ROW_HEIGHT * (MAX_INPUTS - inputs));
        }
        ...
        private void SetColumns_BitMode_MaskValue()
        {
            ...
            m_dgvBitFieldConfig.ClientSize = new Size(DGV_WIDTH, DGV_HEIGHT);
        ...
        private void CheckAndSaveAllParams()
        {
            ...
            else    // No error is reported, save all the parameters to symbol
            {
                ...
                m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.NUMINPUTS, nInputs.Value.ToString());
                m_Component.CommitParamExprs();
                
                // Save Bus parameter
                if (chBus.Checked)
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.BUS, Resource.TRUE);
                }
                else
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.BUS, Resource.FALSE);
                }
                ...
                if (chInterrupt.Checked)
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.INTRR, Resource.TRUE);
                }
                else
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.INTRR, Resource.FALSE);
                }
                SetMaskValue();
        ...
        private void SetMaskValue()
        {
            ...
            m_Component.SetParamExpr(CyStatusReg_v1_70.StatusRegParameters.MASKVALUE, maskval.ToString());
            m_Component.CommitParamExprs();
        }
