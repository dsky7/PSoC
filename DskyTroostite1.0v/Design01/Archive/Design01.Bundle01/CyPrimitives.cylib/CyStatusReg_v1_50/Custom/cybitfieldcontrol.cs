    ...
    public partial class CyBitfieldControl : UserControl
    {
        ...
        private void SetColumnsVisibility(ICyInstEdit_v1 inst)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                rowobs[INPUT_ROWS_START + 0] = "Bit" + i.ToString() + "Mode";
                try
                {
        ...
        private void CyBitfieldControl_VisibleChanged(object sender, EventArgs e)
        {
            if (!this.Visible)
            {
                m_Component.SetParamExpr(CyStatusReg_v1_50.StatusRegParameters.NUMINPUTS, m_nudInputs.Value.ToString());
                m_Component.CommitParamExprs();
            }
            else
            {
                try
                {
                    m_nudInputs.Value = Convert.ToInt16(m_Component.GetCommittedParam(CyStatusReg_v1_50.StatusRegParameters.NUMINPUTS).Value);
                }
                catch (Exception) { }
            }
        }
		...
		private void CheckAndSaveInputValue()
		{
            ...
            try
            {
                paraminputs = Convert.ToInt16(m_Component.GetCommittedParam(CyStatusReg_v1_50.StatusRegParameters.NUMINPUTS).Value);
            }
            catch (Exception) { }
